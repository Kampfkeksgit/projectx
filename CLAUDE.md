...
│       │   │                       # ||spoiler||/[label](url)/> quote/- list + # ## ### Header NUR in Content+V2, nicht in Embed-Desc — Discord-konform)
...
- **Datum:** 2026-07-13
- **SEO-Content — Docs-Modulseiten + Feature-Landingpages (kein Schema-Change):** Aufbauend aufs SEO-Fundament. **(B) Docs:** `docs/modules.md` war eine Sammelseite (1 URL für alle Module) → jetzt zusätzlich **eine Seite pro Modul** unter `docs/modules/<slug>.md` (12 Top-Module: welcome/tickets/moderation/leveling/reaction-roles/stats/music/economy/giveaways/backup/minecraft/tempvoice), jede mit keyword-optimiertem H1 + Intro (GitBook zieht Titel/Meta daraus) + „Was es macht"/„Einrichten". In [SUMMARY.md](docs/SUMMARY.md) unter „Module" verschachtelt + aus [modules.md](docs/modules.md) verlinkt. Weitere Module nach demselben Muster ergänzbar. **(C) Feature-Landingpages:** datengetrieben — [seo/featurePages.js](frontend/src/seo/featurePages.js) (5 Top-Keywords: discord-ticket-bot/welcome-bot/leveling-bot/moderation-bot/music-bot) + [FeatureLanding.vue](frontend/src/pages/FeatureLanding.vue) (Hero/Features/FAQ/CTA, `useSeo` + **FAQPage-JSON-LD** für Rich Results). Routen im [router](frontend/src/router/index.js) aus `FEATURE_PAGES` generiert; 5 URLs in [sitemap.xml](frontend/public/sitemap.xml). Frontend-Build grün. **Noch offen:** restliche Module als Docs-Seiten, mehr Feature-Pages, echte `hreflang`/`/de//tr/`-URLs, Prerendering.
