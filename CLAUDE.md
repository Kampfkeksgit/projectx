...
- `wavelink==3.4.1` — Lavalink-Client fürs Music-Modul ([music.py](bot/cogs/music.py)). **Optional zur Laufzeit:** der Cog ist hinter `WAVELINK_AVAILABLE` + `config.LAVALINK_HOST` gegated → ohne wavelink/Lavalink bleibt das Modul inert (kein Crash). Braucht zusätzlich einen **Lavalink-Server** (eigener Container in [docker-compose.yml](docker-compose.yml), **gepinnt auf `lavalink:4.0.8`** — passend zum `VoiceState`-Protokoll von wavelink 3.4.1; das rollende `:4` zieht 4.2.x, das ein `channelId`-Feld verlangt, das wavelink 3.4.1 nicht schickt → Voice-Connect-Timeout. **wavelink + Lavalink immer im Gleichschritt bumpen.**).
...
| Music: Bot joint kurz den Voice-Channel und leavt sofort wieder; Lavalink-Log `Field 'channelId' is required ... missing at $.voice` + `exceeded the timeout of 60.0 seconds` | wavelink↔Lavalink-**Protokoll-Mismatch**: der rollende `:4`-Tag zog Lavalink 4.2.x, dessen `VoiceState` ein `channelId`-Feld verlangt, das wavelink 3.4.1 nicht sendet → Player-Update abgelehnt → Voice-Connect-Timeout → Bot leavt. **Fix aktiv:** Lavalink in [docker-compose.yml](docker-compose.yml) auf **`4.0.8`** gepinnt (passt zu wavelink 3.4.1). Stack neu deployen (`docker compose pull lavalink` + up). Für neuere Lavalink: wavelink in [requirements.txt](bot/requirements.txt) im Gleichschritt hochziehen. |
