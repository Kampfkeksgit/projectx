...
│   │                           # meldet via report_error → POST /api/bot/errors; globaler on_error fängt Event-Exceptions (→ error_log/Admin-Monitoring)
...
│   │   ├── general_config.py   # get_settings/get_embed_color/get_language/get_timezone — pro-Guild Allgemein-Settings
│   │   │                       # via GET /api/bot/guilds/:id/settings/general, 60s Cache; get_embed_color(...,fallback)
│   │   │                       # → discord.Color (tönt Marken-Embeds der Cogs; semantische Farben bleiben). invalidate()
│   │   ├── bot_i18n.py         # t(lang,key,**kw) + lang_for(...) — i18n für Nicht-Games-Cogs (Sprache aus general_config),
│   │   │                       # geteilte STRINGS-Tabelle (en/de/tr/ru/pl, 245 Keys, namespaced suggest./verify./poll./
│   │   │                       # giveaway./eco./app./ticket./mod./raid./bday./count./rm./slash./tv.), nutzt
...
│       ├── logs.py             # 23 Listener: join/leave(+kick)/ban/unban/message_edit/message_delete/bulk_delete/
│       │                       # member_update(+boost)/channel_*/role_*/invite_*/thread_*/emojis/stickers/webhooks/
│       │                       # automod/voice_state → Embed. Routing über 4 Kategorie-Kanäle (member/message/voice/
│       │                       # server, Fallback log_channel_id). Filter _is_actor_ignored (ignored_role/user_ids +
│       │                       # ignore_bots), _is_ignored (Channel). show_executor → _find_executor via Audit-Log
│       │                       # („wer war es", best-effort, view_audit_log). Log-Text bleibt EN (Admin-Audit).
...
│       │                       # Sendet im guildMeta-Seed zusätzlich Server-Inhaber (owner_id + Name/Avatar,
│       │                       # cache/fetch_user) → guilds-Owner-Spalten für den Admin-Inspektor (v55).
...
│       │                       # Level via /leveling/user) vor dem Eintrag. 30s-Loop lost Gewinner aus; 2. 30s-Loop (_post_pending) postet
│       │                       # Dashboard-erstellte Giveaways. Embed zeigt Voraussetzungen + Host.
│       ├── music.py            # Music-Modul (Pro) via wavelink 3.x → Lavalink. Slash /play /skip /stop /pause /resume /queue
│       │                       # /nowplaying /volume /loop /shuffle. Quellen: SoundCloud/Bandcamp/Twitch/HTTP-Radio — KEIN YouTube
│       │                       # (Lavalink-seitig deaktiviert + hier defensiv geblockt). Pusht Live-Snapshot (PUT /music/state) +
│       │                       # pollt Dashboard-Steuerung (GET/DELETE /music/commands). DJ-Rollen-Gate, Auto-Disconnect, Default-Volume.
│       │                       # INERT ohne wavelink-Install ODER ohne config.LAVALINK_HOST (kein Crash). Geladen in main.py.
...
│       ├── partner_resolve.py  # @tasks.loop(5min): pollt GET /api/bot/partners/unresolved; kind=user → fetch_user
│       │                       # (Name/Avatar), kind=guild → fetch_invite (Servername/Icon/Mitgliederzahl/Guild-ID,
│       │                       # auch ohne Mitgliedschaft). PUT /api/bot/partners/:id/resolved. Für die /partners-Seite.
...
│       ├── automod_sync.py     # @tasks.loop(90s): pollt GET /api/bot/automod/pending (dirty). Bei automod_native spiegelt es
│       │                       # Banned-Words/Invite/Link → KEYWORD-, Anti-Spam → SPAM-, Anti-Mention → MENTION_SPAM-Regel in
│       │                       # ECHTE Discord-AutoMod-Regeln (guild.create_automod_rule/edit/delete, block_message + exempt roles/
│       │                       # channels). Verwaltet nur eigene Regeln (feste Namen). Schaltet den „Uses AutoMod"-Badge frei.
│       │                       # Braucht MANAGE_GUILD; meldet Status-Code (PUT .../automod/applied).
│       ├── bot_profile.py      # @tasks.loop(60s): pollt GET /api/bot/botprofile/pending (dirty), wendet pro-Server Bot-
│       │                       # Nickname + Avatar via guild.me.edit(nick,avatar) an (server-spezifisches Bot-Profil),
│       │                       # lädt Avatar-Bild (aiohttp, ≤10MB), meldet Status-Code zurück (PUT .../botprofile/applied).
│       │                       # nick braucht CHANGE_NICKNAME; leer = Reset. Für die Bot-Profil-Seite (Free).
...
│   │   ├── giveaways.js        # /api/guilds/:id/giveaways (GET list + POST create + DELETE, cookie) — POST = Dashboard-Erstellung (duration_seconds → ends_at)
│   │   ├── music.js            # /api/guilds/:id/music (settings GET/PUT) + /music/state (GET Live-Snapshot) + /music/control (POST Steuerung), cookie — Music (Pro)
...
│   │   │                       # + /marketplace (Template-Marketplace: POST /publish, POST /upload (Datei-Upload ohne Source-Guild),
│   │   │                       # GET list, GET /:id/export (Download JSON), PUT /:id/status, DELETE /:id — Audit MARKETPLACE_*)
...
│   ├── state/
│   │   └── botStats.js         # In-Memory Cache für Bot-Stats (guild_count, user_count, started_at, latency_ms, version)
│   │                           # getPublicStats() (Landing) + getBotHealth() (Admin-Monitoring: online/uptime/latency/last-seen)
...
│       │   ├── mobile.css      # Globale Mobile-Schicht — Selektoren als **html.mobile-ui** (erhöhte Spezifität, damit sie
│       │   │                   # scoped Component-Styles zuverlässig überschreiben; sonst gewinnt die später geladene Route-CSS).
│       │   │                   # Deckt ALLE Seiten ab: --nav-height→Top-Bar-Höhe, Overview/Container einspaltig, alle
...
│       │   ├── useAutoRefresh.js # useAutoRefresh(refetchFn, {isDirty?,enabled?,interval?}) — hält Seiten-Daten aktuell ohne Reload:
│       │   │                   # refetch bei Tab-Fokus/visibilitychange + Poll (default 25s), nie wenn Tab versteckt, nie überlappend,
│       │   │                   # SKIPPT solange isDirty() true (schützt ungespeicherte Formulare). In (fast) allen Modul-Seiten verdrahtet.
...
│       │   └── useSeo.js       # Per-Route-SEO (SPA ohne SSR): useSeo(()=>({title,description})) setzt document.title +
│       │                       # meta description/canonical/OG/Twitter reaktiv (i18n-Sprachwechsel folgt); resetSeo(path) in
│       │                       # router.afterEach setzt Default. Verdrahtet in Landing/Team/Changelog (öffentliche Seiten)
│       ├── components/
...
│       │   │                   # Blöcke Text/Trenner/Bild/Section[Text+Thumbnail rechts]; format/accent_color/blocks im selben *_embed-JSON)
│       │   ├── embedPlaceholders.js # Shared PLACEHOLDERS-Liste + insertAtCaret-Helper (Welcome/Leave/EmbedEditor)
│       │   ├── DiscordMessagePreview.vue  # Mock-Discord-Bubble — plain ODER embed-mode ODER Components-V2-Container (embed.format);
│       │   │                       # rendert Discord-Markdown in der Vorschau (renderMarkdown: **fett**/*kursiv*/__u__/~~s~~/`code`/
│       │   │                       # ||spoiler||/[label](url)/> quote/- list/-# subtext + # ## ### Header NUR in Content+V2, nicht in Embed-Desc — Discord-konform).
│       │   │                       # Mockt in der Vorschau auch Ticket-Tokens {number}→42 / {category}→Support (sonst blieben sie literal)
...
│           ├── Partners.vue         # /partners (öffentlich, kein Auth) — Partner-Seite (Discord-User + Partner-Guilds), Karten mit Avatar/Icon,
│           │                        # Kind-Badge (Creator/Server), Mitgliederzahl + „Server beitreten"-Button (Guild-Invite), lädt GET /api/public/partners
...
│           ├── Overview.vue         # /dashboard/:guild_id (Übersicht — 34 Modul-Cards mit Live-Status)
│           ├── General.vue          # /dashboard/:guild_id/general (Allgemein: Sprache/Zeitzone/Standard-Embed-Farbe/Dashboard-Theme dark|light; wendet Theme live via data-app-theme an)
│           ├── BotProfile.vue       # /dashboard/:guild_id/botprofile (Free) — pro-Server Bot-Nickname + Avatar (URL + Vorschau), zeigt Anwende-Status/Fehler vom Bot; lädt/speichert /botprofile
...
│           ├── Admin.vue           # /admin (Owner ODER Staff — Router-Guard requiresAdmin; Tabs Overview/Analytics/Premium/Health/Users/Guilds/Audit/Jobs/Errors/Marketplace/Team/Partners/Changelog/System + Staff (owner-only)).
│           │                       # Fetcht /api/admin/access → filtert sichtbare Tabs nach view-Recht + gated Aktions-Controls nach can(tab,'manage'). Staff-Tab = Rechte-Matrix pro Admin.
│           │                       # Marketplace (Vorlagen-Verwaltung: JSON-Datei hochladen → erscheint im Marketplace, je Vorlage Export-Download/Status/Löschen),
...
- `wavelink==3.5.2` — Lavalink-Client fürs Music-Modul ([music.py](bot/cogs/music.py)). **Optional zur Laufzeit:** der Cog ist hinter `WAVELINK_AVAILABLE` + `config.LAVALINK_HOST` gegated → ohne wavelink/Lavalink bleibt das Modul inert (kein Crash). Braucht zusätzlich einen **Lavalink-Server** (eigener Container in [docker-compose.yml](docker-compose.yml), **gepinnt auf `lavalink:4.2.2`**). **⚠️ wavelink + Lavalink müssen im Gleichschritt zueinander passen:** Lavalink 4.2.x verlangt im `VoiceState` ein `channelId`-Feld, das erst wavelink **≥ 3.5** schickt (3.4.1 nicht → Voice-Connect-Timeout, Bot leavt sofort); ältere Lavalink 4.0.x (lavaplayer 2.2.2) liefern dagegen **404 auf SoundCloud-HLS-Streams**. 4.2.2 (lavaplayer 2.2.6) + wavelink 3.5.2 ist die passende Kombi. Beim Bumpen einer Seite die andere mitziehen.
...
```
Services: `backend` (intern :3000, nicht published), `bot`, `frontend` (nginx serviert die SPA + reverse-proxyt `/api` → backend), `lavalink` (Audio-Server fürs Music-Modul, `ghcr.io/lavalink-devs/lavalink:4`, mountet [lavalink/application.yml](lavalink/application.yml) — **YouTube bewusst nicht installiert**; entfernen + Bot-`LAVALINK_*`-Env leeren, um ohne Musik zu deployen). **Kein `db`-Service** (SQLite ist dateibasiert; Persistenz über das Named Volume `projectx-data` an `/data`). Alle Dockerfiles liegen im Root und builden mit `context: .` (COPY aus `backend/`,`bot/`,`frontend/`). **VITE_*-Variablen sind Build-Args** (in `docker-compose.yml` unter `frontend.build.args`), nicht Runtime-Env. `VITE_BACKEND_URL=/api` hält Frontend+API same-origin (keine CORS-/Cookie-Probleme).
...
- `BOT_VERSION` — optional; App-Versions-String im Admin-Health-Panel (Fallback: `APP_VERSION`, dann `1.0.0`). Wird vom `presence`-Cog mit `PUT /api/bot/stats` gepusht. Zusätzlich meldet der Cog automatisch die **echten Laufzeit-Versionen** `py_version` (Python) + `dpy_version` (discord.py) — immer aktuell zum laufenden Bot, ohne manuelles Pflegen.
- `APP_VERSION` — optional; gemeinsamer Versions-String für Bot **und** Backend (z. B. Git-Short-SHA beim Deploy gesetzt, in [docker-compose.yml](docker-compose.yml) an beide Services durchgereicht) → das Health-Panel spiegelt automatisch den echten Build. Bot nutzt es als `BOT_VERSION`-Fallback, Backend als `backend.version`-Quelle (vor `package.json`).

> ⚠️ **Presence-Intent (kein Env, sondern Portal-Toggle):** Das `stats`-Cog zählt Online/Offline über `member.status` — dafür ist das **privilegierte Presence-Intent** nötig. In [main.py](bot/main.py) ist `intents.presences = True` gesetzt; zusätzlich muss es im **Discord Developer Portal** (Bot → Privileged Gateway Intents → Presence Intent) aktiviert werden, sonst zählt der Bot alle Member als offline. Auto-Create von Stats-Channels braucht außerdem `MANAGE_CHANNELS`, Temp-Voice zusätzlich `MOVE_MEMBERS`, Server-Backup/Restore zusätzlich `MANAGE_ROLES` (bereits enthalten) + `MANAGE_GUILD` (Server-Name/-Icon) — die Invite-Bitmask wurde auf `352332854` erhöht (= `268446742 | MOVE_MEMBERS | MANAGE_GUILD | CHANGE_NICKNAME` — CHANGE_NICKNAME für das Bot-Profil-Modul).
...
- **Music / Lavalink (alle optional — `music`-Cog ist ohne `LAVALINK_HOST` inert):**
  - `LAVALINK_HOST` — Hostname des Lavalink-Servers (im Docker-Stack `lavalink`). **Leer = Music-Modul komplett aus.**
  - `LAVALINK_PORT` (default `2333`), `LAVALINK_PASSWORD` (default `youshallnotpass` — in Prod ändern, muss zum Lavalink-`application.yml`/`LAVALINK_SERVER_PASSWORD` passen), `LAVALINK_SECURE` (`true` für wss/https, default `false`).
  - `MUSIC_POLL_INTERVAL` — Poll-Intervall für Dashboard-Steuerbefehle in Sekunden (default `4`, hartes Minimum 2).
...
| `/api/admin/*` | [backend/routes/admin.js](backend/routes/admin.js) | `requireSession` + `requireAdmin` (Owner ODER Staff) + per-Tab `PERM_RULES`-Gate; `/staff*` owner-only |
...
- `PUT /api/guilds/:id/settings/logs` body matches → `{ success: true, message, settings }`. Log-Settings-Shape (v50): `{ enabled, log_channel_id, member_log_channel_id, message_log_channel_id, voice_log_channel_id, server_log_channel_id, log_joins, log_leaves, log_message_edits, log_message_deletes, log_member_bans, log_member_unbans, log_member_updates, log_boosts, log_channels, log_roles, log_voice, log_invites, log_threads, log_emojis, log_bulk_delete, log_automod, log_webhooks, log_ignored_channel_ids: string[], ignored_role_ids: string[], ignored_user_ids: string[], ignore_bots, show_executor }`. Kategorie-Kanäle fallen leer auf `log_channel_id` zurück.
...
- `PUT /api/bot/stats` body `{ guild_count, user_count, started_at, latency_ms?, version?, py_version?, dpy_version? }` → `{ success: true }`. Befüllt den In-Memory-Cache in [state/botStats.js](backend/state/botStats.js) für die Landing-Page **und das Admin-Health-Panel** (`getBotHealth()`). `started_at` ist unix-seconds (Bot setzt das einmal beim ersten `on_ready`); `latency_ms`/`version`/`py_version` (Python)/`dpy_version` (discord.py) werden vom `presence`-Cog mitgeschickt (Python/discord.py automatisch aus der Laufzeit). Wird zusammen mit `presence` alle 5 Minuten aufgerufen.
- `POST /api/bot/errors` body `{ level?, context?, message, stack?, guild_id? }` → `{ success: true }`. Schreibt eine Bot-Exception ins zentrale `error_log` (`source = 'bot'`). Vom Bot in `on_command_error` + globalem `on_error` (`report_error` in [main.py](bot/main.py)) aufgerufen.
...
- `GET  /api/admin/guilds?search=&present=&limit=&offset=` → `{ success, guilds: [{ id, guild_name, guild_icon_url, bot_present, blocked, blocked_reason, blocked_at, blocked_until, premium_tier, premium_source, premium_until, premium_effective, created_at }], total }`. `present ∈ {''|present|absent}` filtert nach Bot-Präsenz (`bot_present`) — Dropdown im Admin-Server-Tab.
...
- `GET /api/admin/guilds/:guild_id/inspect` → `{ success, inspect: { id, guild_name, guild_icon_url, bot_present, blocked, blocked_reason, blocked_until, premium_tier, premium_source, premium_until, premium_effective, dashboard_members, created_at, owner: { id, username, avatar_url } | null, modules:[{key,kind,enabled,configured,count?}] } }`. Read-only Modul-/Premium-/Presence-Snapshot (Support-Tool). `owner` = Server-Inhaber (bot-synced via guild_sync in die `guilds`-Owner-Spalten, Fallback `users`-Join; das Frontend zeigt Avatar/Name/ID + `discord.com/users/:id`-Link zum Kontaktieren). 404 wenn Guild unbekannt. `getGuildInspect`.
...
- **Team/Credits:** `GET /api/admin/team` → `{ members }`; `POST /api/admin/team` body `{ name?, role?, discord_id?, avatar_url?, bio?, position?, socials? }` → `{ member }` (**Name ODER discord_id Pflicht**, sonst 400 `name_required`; Audit `ADMIN_TEAM_CREATE`); `PUT /api/admin/team/:id` (404 `not_found`, Audit `ADMIN_TEAM_UPDATE`); `DELETE /api/admin/team/:id` (Audit `ADMIN_TEAM_DELETE`). Öffentlich: `GET /api/public/team` → `{ members: [{ id, discord_id, discord_username, name, role, avatar_url, bio, socials, position }] }` (Cache 60s, von [Team.vue](frontend/src/pages/Team.vue) geladen). `socials` nur whitelisted Keys; Frontend rendert nur `https?://`-Werte als Link. Bot (`X-Bot-Token`): `GET /api/bot/team/unresolved` → `{ members: [{ id, discord_id }] }` (discord_id gesetzt, noch nicht aufgelöst); `PUT /api/bot/team/:id/resolved` body `{ username, name, avatar_url }` (Bot schreibt aufgelösten Discord-User zurück; füllt Name/Avatar nur wenn leer, setzt `discord_username` als Resolved-Marker).
- **Admin-Rechte / Staff (Owner-only für Staff-Verwaltung):** `GET /api/admin/access` → `{ is_owner, is_staff, permissions: {tab:'view'|'manage'}, tabs }` (eigener Zugriff jedes Admins — steuert Tab-Sichtbarkeit + Gating; **jeder Admin**). `GET /api/admin/staff` → `{ staff, tabs }`; `POST /api/admin/staff` body `{ user_id, note?, permissions }` → `{ member }` (400 `is_owner`/`invalid_user`; Audit `ADMIN_STAFF_UPSERT`); `DELETE /api/admin/staff/:user_id` (404 `not_found`, Audit `ADMIN_STAFF_REMOVE`) — **owner-only**. Alle anderen `/api/admin/*`-Routen laufen durch `requireAdmin` (Owner ODER Staff) + den zentralen `PERM_RULES`-Gate (Method+Pfad → Tab/Level; Staff braucht das jeweilige `view`/`manage`). `is_admin` (Owner||Staff) steht zusätzlich in `/auth/me` + `/auth/callback` (Frontend-Guard `requiresAdmin` + Nav-Link).
- **Partner:** `GET /api/admin/partners` → `{ partners }`; `POST /api/admin/partners` body `{ kind ∈ {user|guild}, discord_id?, invite_url?, name?, description?, avatar_url?, position? }` → `{ partner }` (user braucht `discord_id` ODER `name`, guild braucht `invite_url`, sonst 400 `partner_invalid`; Audit `ADMIN_PARTNER_CREATE`); `PUT /api/admin/partners/:id` (404 `not_found`, Audit `ADMIN_PARTNER_UPDATE`; Kind-/ID-/Invite-Wechsel resettet den Resolve-State); `DELETE /api/admin/partners/:id` (Audit `ADMIN_PARTNER_DELETE`). Öffentlich: `GET /api/public/partners` → `{ partners: [{ id, kind, discord_id, guild_id, invite_url, resolved_name, name, description, avatar_url, member_count, position }] }` (Cache 60s, von [Partners.vue](frontend/src/pages/Partners.vue) geladen). Bot (`X-Bot-Token`): `GET /api/bot/partners/unresolved` → `{ partners: [{ id, kind, discord_id, invite_url }] }`; `PUT /api/bot/partners/:id/resolved` body `{ resolved_name, name?, avatar_url?, guild_id?, member_count? }` (user via `fetch_user`, guild via `fetch_invite`; füllt Name/Avatar nur wenn leer, setzt `resolved_name` als Marker).
- **Changelog-Publisher:** `GET /api/admin/changelog` → `{ entries }` (alle inkl. Entwürfe); `POST /api/admin/changelog` body `{ version?, title, body?, published?, entry_date? }` → `{ entry }` (400 `title_required`, Audit `ADMIN_CHANGELOG_CREATE`); `PUT /api/admin/changelog/:id` (404 `not_found`, Audit `ADMIN_CHANGELOG_UPDATE`); `DELETE /api/admin/changelog/:id` (Audit `ADMIN_CHANGELOG_DELETE`). Öffentlich: `GET /api/public/changelog` → `{ entries: [{ id, version, title, body, published, entry_date, created_at, updated_at }] }` (nur veröffentlichte, Cache 60s, von [Changelog.vue](frontend/src/pages/Changelog.vue) geladen). `entry_date`/`*_at` = unix-seconds. db.js: `getChangelogEntries`/`getPublishedChangelog`/`createChangelogEntry`/`updateChangelogEntry`/`deleteChangelogEntry`.
...
**General** (Cookie required, settings-only, Free) — `{ success, settings }` / PUT `{ success, message, settings }`. Audit `UPDATE_GENERAL_SETTINGS`.
- `GET|PUT /api/guilds/:id/general` → `{ language ∈ {en|de|tr|ru|pl}, timezone (IANA, sonst UTC), embed_color (#RRGGBB), dashboard_theme ∈ {dark|light} }`. Bot-GET (X-Bot-Token): `GET /api/bot/guilds/:id/settings/general` → rohes Objekt. **Kein `enabled`-Toggle** (immer aktiv → nicht in `FLAG_MODULE_TABLES`).
...
- `GET /api/guilds/:id/giveaways` → `{ giveaways: [{ id, channel_id, message_id, prize, winners_count, ends_at, ended, host_id, description, required_role_ids, min_account_age_days, min_member_days, min_level }] }`; `POST /api/guilds/:id/giveaways` body `{ channel_id, prize, winners_count, duration_seconds, description?, required_role_ids?, min_account_age_days?, min_member_days?, min_level? }` → `{ success, giveaway }` (Dashboard-Erstellung, Bot postet es; VALIDATION→400, Audit `GIVEAWAY_CREATE`); `DELETE /:id`. Audit `GIVEAWAY_DELETE`.
...
- Cookie (`requireSession` + `requireGuildAccess`, Premium-Gate `backup`): `GET /api/guilds/:id/backups` → `{ success, snapshots: [{ id, name, guild_name, guild_icon_url, channels_count, roles_count, created_at }], jobs: [{ id, type, status, backup_id, mode, message, created_at, updated_at }] }`; `GET /api/guilds/:id/backups/templates` → `{ success, sources: [{ guild_id, guild_name, guild_icon_url, snapshots: [...] }] }` (Snapshots **aller anderen vom User verwalteten** Server — für Server-Vorlagen); `GET /api/guilds/:id/backups/:backup_id` → `{ success, snapshot }` (voller Snapshot **inkl. `data`-Blob** für die Dashboard-Vorschau vor dem Restore, 404 wenn fehlt); `POST /api/guilds/:id/backups` → legt Snapshot-Job an, `{ success, job }` (Audit `BACKUP_SNAPSHOT_REQUEST`); `POST /api/guilds/:id/backups/apply-template` body `{ source_guild_id, backup_id, mode, parts? }` → wendet eine Vorlage von einem anderen Server an (verifiziert User-Zugriff auf BEIDE Guilds; legt Restore-Job auf der Ziel-Guild an), `{ success, job }`, 403/404 (Audit `BACKUP_APPLY_TEMPLATE`); `POST /api/guilds/:id/backups/:backup_id/restore` body `{ mode ∈ {missing|mirror}, parts? }` → Restore-Job, `{ success, job }`, 404 wenn Snapshot fehlt (Audit `BACKUP_RESTORE_REQUEST`). **`parts`** (optional, beide Endpoints) = `{ roles, channels, server_name, server_icon }` Booleans — schränkt ein, welche Teile wiederhergestellt werden (fehlt/leer → alle Teile); `DELETE /api/guilds/:id/backups/:backup_id` → `{ success }`, 404 (Audit `BACKUP_DELETE`).
...
- **Template-Marketplace** (v34): `GET /api/guilds/:id/backups/marketplace` → `{ success, templates: [{ id, name, description, category, guild_name, guild_icon_url, channels_count, roles_count, uses, created_at }] }` (approved, Browse); `GET /api/guilds/:id/backups/marketplace/:template_id` → `{ success, template }` (inkl. `data`, Vorschau); `POST /api/guilds/:id/backups/apply-marketplace` body `{ template_id, mode, parts? }` → Restore-Job auf Ziel-Guild (Pro, zählt `uses` hoch, Audit `BACKUP_APPLY_MARKETPLACE`). **Owner-only unter `/api/admin/marketplace`** (`requireSession`+`requireOwner`): `POST /publish` body `{ source_guild_id, backup_id, name?, description?, category? }` (veröffentlicht einen Snapshot als Vorlage, **strippt Member-Overwrites**, Audit `MARKETPLACE_PUBLISH`); `POST /upload` body `{ name?, description?, category?, guild_name?, guild_icon_url?, data }` (lädt eine Vorlage **ohne Source-Guild** hoch — `data` ist ein Snapshot-Blob `{server,roles,channels}`, roh ODER in einer exportierten Datei unter `data` verschachtelt; `source_guild_id = null`; validiert dass Rollen/Kanäle vorhanden sind, sonst 400 `invalid_template`/`empty_template`; Audit `MARKETPLACE_UPLOAD`); `GET /marketplace` (alle Stati, Verwaltung); `GET /marketplace/:id/export` (Download des vollen Templates inkl. `data` als re-importierbare JSON-Datei, `Content-Disposition: attachment`, Audit `MARKETPLACE_EXPORT`); `PUT /marketplace/:id/status` body `{ status }`; `DELETE /marketplace/:id` (Audit `MARKETPLACE_DELETE`). DB-Helfer in [db.js](backend/db.js): `createMarketplaceTemplate`/`getMarketplaceTemplates`/`getMarketplaceTemplate`/`getAdminMarketplaceTemplates`/`deleteMarketplaceTemplate`/`setMarketplaceTemplateStatus`/`incrementMarketplaceUses`.
...
- Migrations: [backend/migrations.js](backend/migrations.js)
  - **Aktuelle Schema-Version: `55`**
  - `CURRENT_SCHEMA_VERSION` Konstante steuert Upgrades.
  - `applyMigrations(from, to)` mappt Versionsnummern → Migration-Funktionen (`migrationV1`, …, `migrationV55`). v23–v55 nutzen den `runSchemaBatch(version, statements)`-Helper.
  ...
  - `migrationV41` (Rollen-Menü-Live-Update, idempotenter ALTER + Mirror): `guild_role_menus.dirty INTEGER DEFAULT 0`. Wird bei jeder Dashboard-Bearbeitung eines Menüs (`updateRoleMenu`) auf `1` gesetzt; `getPendingRoleMenus()` liefert jetzt auch gepostete, aber dirty-Menüs (`(message_id IS NULL/'' ) OR dirty = 1`). Der `rolemenus`-Cog editiert bei vorhandener `message_id` die **bestehende Nachricht in-place** (statt zu duplizieren) und ruft `setRoleMenuMessage` → `dirty = 0`. Behebt: Inhalts-/Options-/Embed-Änderungen an einem bereits geposteten Menü erschienen vorher nicht in Discord (nur Channel-Wechsel triggerte einen Repost).
  - `migrationV42` (Giveaway-Überarbeitung, 6 idempotente ALTERs auf `guild_giveaways` + Mirror): `host_id`, `description`, `required_role_ids` (JSON), `min_account_age_days`, `min_member_days`, `min_level`. Ermöglicht Dashboard-Erstellung (Giveaway ohne `message_id` → Bot postet via `getPendingGiveaways`-Loop) und Teilnahme-Voraussetzungen (Rollen/Kontoalter/Serverzugehörigkeit/Level), die der Bot beim Button-Klick prüft. `createGiveaway` nimmt die Felder + `host_id`/`message_id`; neue Helfer `getPendingGiveaways`/`endGiveawayNow`.
  ...
  - `migrationV47` (Changelog-Publisher, Owner-Admin): legt `changelog_entries` an (`id` UUID, `version`, `title` (Pflicht), `body` (Markdown/Klartext), `published`, `entry_date`/`created_at`/`updated_at` unix-seconds; `idx_changelog_date`). Idempotent + Mirror. db.js: `getChangelogEntries` (Admin, alle inkl. Entwürfe), `getPublishedChangelog(limit)` (öffentlich, nur `published=1`), `createChangelogEntry`/`updateChangelogEntry`/`deleteChangelogEntry`. Owner verwaltet Einträge im Admin-Tab „Changelog"; veröffentlichte erscheinen auf der öffentlichen Seite `/changelog`.
  - `migrationV55` (Server-Inhaber im Inspektor): 3 idempotente ALTERs auf `guilds` — `owner_id`, `owner_username`, `owner_avatar_url` (vom Bot-Cog `guild_sync` gesetzt: `owner_id` immer, Name/Avatar best-effort aus Cache bzw. `fetch_user`). Der Guild-Seed in `replaceGuildChannels`/`replaceGuildRoles` schreibt sie mit (COALESCE/NULLIF → leere Werte überschreiben nie einen gesetzten Namen). `getGuildInspect` liefert `owner` (mit `users`-Join-Fallback); der Admin-Inspektor zeigt Avatar/Name/ID + `discord.com/users/:id`-Link.
  - `migrationV54` (Natives Discord-AutoMod): idempotenter ALTER `guild_moderation_settings.automod_native` + neue Tabelle `guild_automod_state` (`guild_id` PK, `dirty`, `status`, `status_message`; FK CASCADE). Idempotent + Mirror. Bei aktivem Toggle spiegelt der Bot-Cog `automod_sync` die Moderations-Config (Banned-Words/Invite/Link → KEYWORD-Regel, Anti-Spam → SPAM-Regel, Anti-Mention → MENTION_SPAM-Regel) in **echte Discord-AutoMod-Regeln** via `guild.create_automod_rule`/`rule.edit`/`rule.delete` — das schaltet den **„Uses AutoMod"-Badge** frei (Bot-seitiges Filtern allein zählt nicht). Jede Moderations-Speicherung setzt `dirty=1`; db.js: `markAutomodDirty`/`getPendingAutomodSync`/`setAutomodApplied`/`getAutomodState`.
  - `migrationV53` (Bot-Profil pro Server): legt `guild_bot_profile` an (`guild_id` PK, `nickname`, `avatar_url`, `dirty`, `status`, `status_message`; FK CASCADE). Idempotent + Mirror. Ein Admin setzt Nickname + Avatar **nur für einen Server**; der Bot-Cog `bot_profile` (@tasks.loop 60s, poll `dirty=1`) wendet sie via `guild.me.edit(nick=…, avatar=…)` an (Discord unterstützt server-spezifische Bot-Profile) und schreibt Status/Fehler-Code zurück. db.js: `BOT_PROFILE_DEFAULTS` + `getBotProfile`/`upsertBotProfile` (Nick-Cap 32, Avatar via `sanitizeUrlLike`, setzt `dirty=1`+`status='pending'`) / `getPendingBotProfiles` / `setBotProfileApplied`. Free-Modul (Route `/api/guilds/:id/botprofile`, ungegated).
  - `migrationV52` (Admin-Rechtesystem): legt `admin_staff` an (`user_id` PK, `permissions` JSON `{tab:'view'|'manage'}`, `note`, `added_by`, `created_at`/`updated_at`). Der Env-Owner (`OWNER_DISCORD_ID`) behält impliziten Vollzugriff + ist der Einzige, der Staff verwaltet. db.js: `ADMIN_PERM_TABS` (14 zuweisbare Tabs) / `ADMIN_PERM_LEVELS` + `getAdminStaff`/`upsertAdminStaff`/`removeAdminStaff`/`isAdminStaff`/`getAdminAccess`/`adminCan` (Owner→alles manage; Staff→sanitisierte Map; `manage` impliziert `view`). Middleware `requireAdmin` (Owner ODER Staff) in [session.js](backend/middleware/session.js) + zentraler `PERM_RULES`-Gate in [admin.js](backend/routes/admin.js) sichert alle 47 Routen pro Tab/Level (unmapped → fail-closed für Staff, Owner passt).
  - `migrationV51` (Partner-Seite): legt `partners` an (`id` UUID, `kind ∈ {user|guild}`, `discord_id` (user-Input) / `guild_id` (guild-resolved) / `invite_url` (guild-Input; user optional Link), `resolved_name` (Resolved-Marker + Wert), `name`/`description`/`avatar_url` (optionale Owner-Overrides), `member_count` (guild-resolved), `position`, `created_at`; `idx_partners_pos`). Idempotent + Mirror. Spiegelt das team_members-Resolve-Muster: `kind='user'` → Bot `fetch_user`, `kind='guild'` → Bot `fetch_invite` (Name/Icon/Mitgliederzahl/Guild-ID, auch wenn der Bot dort kein Mitglied ist). db.js: `PARTNER_KINDS` + `getPartners`/`createPartner`/`updatePartner`/`deletePartner`/`getUnresolvedPartners`/`resolvePartner` (Invite-Normalisierung, Name/Avatar-Fill nur wenn leer, Reset des Resolve-State bei Kind-/ID-/Invite-Wechsel).
  - `migrationV50` (Server-Logs-Ausbau): 15 idempotente ALTERs auf `guild_log_settings` + Mirror. **Kategorie-Kanäle** (jeder fällt auf `log_channel_id` zurück, wenn leer): `member_log_channel_id` (Beitritte/Austritte/Bans/Unbans/Member-Updates/Boosts), `message_log_channel_id` (Edits/Deletes/Bulk-Delete), `voice_log_channel_id` (Voice), `server_log_channel_id` (Channels/Rollen/Invites/Threads/Emojis/AutoMod/Webhooks). **Filter**: `ignored_role_ids` + `ignored_user_ids` (JSON-Snowflake-Arrays — Events dieser Rollen/User werden übersprungen), `ignore_bots` (Bot-Events überspringen). `show_executor` (löst „wer war es" via Audit-Log auf — Best-Effort, braucht `view_audit_log`). **Neue Event-Toggles**: `log_invites`, `log_threads`, `log_emojis`, `log_bulk_delete`, `log_boosts`, `log_automod`, `log_webhooks`. `LOG_DEFAULTS`/`getLogSettings`/`upsertLogSettings` in [db.js](backend/db.js) + Validierung in [modules.js](backend/routes/modules.js) + Bot-GET-Shape in [bot.js](backend/routes/bot.js) erweitert.
  - `migrationV49` (Minecraft-Status als Kanal-Name + Ping): idempotente ALTERs auf `guild_minecraft_servers` (`name_channel_id`, `name_template`, `ping_ms`).
  - `migrationV48` (Changelog → Discord-Ankündigung): idempotenter ALTER `changelog_entries.announced` (Bot postet veröffentlichte, noch nicht angekündigte Einträge in den Channel; bestehende veröffentlichte werden auf `announced=1` gebackfilled).
...
- `guilds` — Discord-Server; `id PK`, `guild_name`, `guild_icon_url`, `enabled`, `bot_present` (1 wenn der Bot aktuell auf diesem Server ist — vom `presence`-Cog gesetzt), `blocked`/`blocked_reason`/`blocked_at`/`blocked_until` (Owner-Sperre; `blocked_until` = Temp-Ban-Ablauf), `premium_tier` (`free|basic|pro`), `premium_source` (`sku|manual|null`), `premium_until` (unix-seconds Ablauf), `owner_id`/`owner_username`/`owner_avatar_url` (v55, Server-Inhaber — bot-synced via `guild_sync`, für den Admin-Inspektor) — Premium-Modul-Gating
...
- `guild_log_settings` — `guild_id PK`, `enabled`, `log_channel_id` (Standard-Kanal) + 4 Kategorie-Kanäle (`member_/message_/voice_/server_log_channel_id`, leer = Standard), 17 per-event-Flags (joins/leaves/edits/deletes/bans/unbans/member_updates/boosts/channels/roles/voice/invites/threads/emojis/bulk_delete/automod/webhooks), Filter (`log_ignored_channel_ids`/`ignored_role_ids`/`ignored_user_ids` JSON, `ignore_bots`, `show_executor`)
...
- `guild_verification_settings` — `guild_id PK`, `enabled`, `channel_id`, `verified_role_id`, `message`, `button_label`, `panel_message_id`, `use_embed` + `embed` (JSON — gestaltetes Panel statt Plain), `dirty` (1 = im Dashboard editiert → Bot rendert das gepostete Panel neu) — Verification
- `guild_role_menus` + `guild_role_menu_options` — Rollen-Menüs (`id` UUID, `menu_type ∈ {buttons|select}`, `exclusive` (Select-Einzelauswahl), `use_embed` + `embed` (JSON — eigenes Embed statt Auto-Liste), `message_id`, `dirty` (1 = im Dashboard editiert → Bot rendert die gepostete Nachricht neu); Optionen: `role_id`/`label`/`emoji`)
...
- `guild_giveaways` + `guild_giveaway_entries` — Giveaways (`id` UUID, `prize`, `winners_count`, `ends_at`, `ended`, `host_id`, `description`, `required_role_ids` JSON, `min_account_age_days`, `min_member_days`, `min_level`; Entries PK `(giveaway_id, user_id)`). Dashboard-erstellte Giveaways haben bis zum Bot-Post kein `message_id`.
- `guild_music_settings` (`guild_id PK`, `enabled`, `dj_role_id`, `default_volume`, `auto_disconnect_seconds`, `max_queue`, `allow_soundcloud`/`allow_bandcamp`/`allow_http`/`allow_twitch` — kein YouTube) + `guild_music_state` (`guild_id PK`, vom Bot gepushter Live-Snapshot: `voice_channel_id`/`current`/`queue` JSON/`paused`/`volume`/`loop_mode`/`position_ms`) + `guild_music_commands` (`id` UUID, `action`, `payload` JSON — Dashboard-Steuerungs-Queue, Bot pollt+löscht) — Music (Pro, Lavalink)
...
- `guild_moderation_settings.automod_native` (v54) + `guild_automod_state` (`guild_id PK`, `dirty`, `status`, `status_message`) — natives Discord-AutoMod. Bei `automod_native=1` erstellt der Bot-Cog `automod_sync` echte AutoMod-Regeln (Wortfilter/Spam/Massen-Erwähnungen) über die API → **„Uses AutoMod"-Badge**. Jede Moderations-Speicherung markiert `dirty`; der Bot reconciled + schreibt Status/Fehler-Code zurück. Braucht **MANAGE_GUILD** (in der Invite-Bitmask).
- `guild_bot_profile` — pro-Server Bot-Profil (`guild_id PK`, `nickname`, `avatar_url`, `dirty` (1=vom Dashboard geändert→Bot wendet an), `status` (ok|error), `status_message` (Fehler-Code für die Dashboard-Übersetzung)). Free. Der Bot-Cog `bot_profile` wendet Nickname + Avatar per `guild.me.edit(nick=…, avatar=…)` an und schreibt Status zurück. Leerer Nickname → Standardname; leeres Avatar → globales Avatar. Braucht **CHANGE_NICKNAME** (in der Invite-Bitmask enthalten).
- `admin_staff` — zusätzliche Dashboard-Admins (`user_id` PK, `permissions` JSON `{tab:'view'|'manage'}`, `note`, `added_by`, `created_at`/`updated_at`). Der Env-Owner (`OWNER_DISCORD_ID`) hat impliziten Vollzugriff + verwaltet als Einziger Staff. `getAdminAccess`/`adminCan` lösen den effektiven Zugriff auf; `ADMIN_PERM_TABS` (14 Tabs) ist Single Source. Enforcement: `requireAdmin` + `PERM_RULES`-Gate in [admin.js](backend/routes/admin.js).
- `partners` — Partner-Einträge für die öffentliche `/partners`-Seite (`id` UUID, `kind ∈ {user|guild}`, `discord_id` (user), `guild_id` (guild-resolved), `invite_url` (guild-Input / user optional Link), `resolved_name` (Resolved-Marker + Wert), `name`/`description`/`avatar_url` (Owner-Overrides), `member_count` (guild), `position`). Owner-verwaltet (Admin → Partner); `getPartners` joint `users` für einen Avatar-Fallback (user kind). Vom Bot-Cog `partner_resolve` aufgelöst: user via `fetch_user`, guild via `fetch_invite` (funktioniert ohne Mitgliedschaft im Partner-Server).
...
- `getLogSettings(guildId)` / `upsertLogSettings(guildId, settings)` — Boolean-Coercion aller per-event-Flags, Kategorie-Kanäle (v50, Snowflake-validiert), JSON-Parse/Stringify für `log_ignored_channel_ids`/`ignored_role_ids`/`ignored_user_ids` (Snowflakes), `ignore_bots`/`show_executor`.
...
| `error loading dynamically imported module .../assets/Xxx-<hash>.js` (MIME `text/html`, 404) nach einem Deploy | **Stale SPA-Cache:** der Browser hält eine alte `index.html`, die auf einen Chunk-Hash zeigt, den der neue Build gelöscht hat → nginx/Cloudflare liefert 404-HTML statt JS. **Fix aktiv:** [nginx.conf](nginx.conf) setzt `Cache-Control: no-cache` auf `index.html` (`location = /index.html`), sodass Browser sie immer revalidieren; zusätzlich lädt [router/index.js](frontend/src/router/index.js) `router.onError` bei einem Chunk-Load-Fehler die Zielroute **einmal** per Full-Reload neu (10s-Guard gegen Loops). Sofort-Workaround für Endnutzer: Hard-Refresh (`Strg+Umschalt+R`). Cloudflare cachte den 404 zusätzlich 4 h — ggf. im CF-Dashboard „Purge Cache" nach dem Deploy. |
...
| Music: „🎵 Musik ist gerade nicht verfügbar (Audio-Server offline)" + Lavalink-Log zeigt `Undertow started on port 8080` | Lavalink las die gemountete `application.yml` nicht (häufig bei Portainer-Git-Stacks greift der relative Bind-Mount nicht) → Default-Port 8080 statt 2333 **und** YouTube an. **Fix aktiv:** Lavalink wird in [docker-compose.yml](docker-compose.yml) per **Env** konfiguriert (`SERVER_PORT=2333`, `LAVALINK_SERVER_SOURCES_YOUTUBE=false`, …) — das überschreibt die Datei und greift auch ohne funktionierenden Mount. Danach Stack neu deployen. Wenn der Bot **vor** Lavalink hochfuhr: er retryt die Node-Verbindung jetzt 12× alle 5s ([music.py](bot/cogs/music.py) `_connect_nodes`); zur Not `docker compose restart bot`. |
| Music: Bot joint kurz den Voice-Channel und leavt sofort wieder; Lavalink-Log `Field 'channelId' is required ... missing at $.voice` + `exceeded the timeout of 60.0 seconds` | wavelink↔Lavalink-**Protokoll-Mismatch**: Lavalink 4.2.x verlangt im `VoiceState` ein `channelId`-Feld, das erst wavelink **≥ 3.5** sendet (3.4.1 nicht) → Player-Update abgelehnt → Voice-Connect-Timeout → Bot leavt. **Fix aktiv:** wavelink auf **3.5.2** ([requirements.txt](bot/requirements.txt)) + Lavalink **4.2.2** ([docker-compose.yml](docker-compose.yml)). Bot-Image **neu bauen** (`--build`, sonst bleibt altes wavelink) + `docker compose pull lavalink`. |
| Music: Bot bleibt im Channel, aber **kein Ton**; Lavalink-Log `Invalid status code for soundcloud stream: 404` (`SoundCloudM3uAudioTrack`) | **lavaplayer-Bug** in alten Lavalink 4.0.x (lavaplayer 2.2.2): SoundCloud-HLS-Streams liefern 404. **Fix aktiv:** Lavalink auf **4.2.2** (lavaplayer 2.2.6) gepinnt — braucht wavelink ≥ 3.5 (siehe Zeile drüber). `docker compose pull lavalink` + `up -d`. |
...
- **Server-Inhaber im Admin-Inspektor (Schema v55):** Der Server-Inspektor zeigt jetzt den **Server-Inhaber** (Avatar, `@username`, ID) + einen **„Profil öffnen"-Link** (`discord.com/users/:id`), um ihn zu kontaktieren.
  ...
  - **Backend:** `getGuildInspect` liefert `owner: { id, username, avatar_url } | null` (mit `users`-Join als Fallback, falls der Owner ein Dashboard-Nutzer ist). bot.js-Routen `PUT .../channels|roles` nehmen die owner-Felder im Body entgegen.
  - **Frontend:** [Admin.vue](frontend/src/pages/Admin.vue) Inspektor-Modal — Owner-Zeile (Avatar/Name/ID + Profil-Link). i18n `admin.inspectOwner*` (4 Keys) in **allen 5 Sprachen** (Parität 1886/Locale).
  - Verifiziert: Migration v55 + DB-Smoke (Owner-Seed via guild_sync, Cache-Miss überschreibt Name nicht) grün, 74/74 Backend-Tests, `guild_sync.py` kompiliert, Frontend-Build grün, i18n-Parität grün. **Hinweis:** Der Owner erscheint erst nach dem nächsten guild_sync (on_ready / 15min-Loop / on_guild_join); ist der Owner nicht im Cache, wird er per `fetch_user` aufgelöst. Bei Servern ohne Sync (Bot nie drauf) bleibt `owner` leer.
- **Datum:** 2026-07-16
- **Natives Discord-AutoMod → „Uses AutoMod"-Badge (Schema v54):** Neuer Toggle im Moderations-Modul „**Natives Discord-AutoMod nutzen**". Ist er an, spiegelt der Bot die Moderations-Config in **echte Discord-AutoMod-Regeln** über die API — das ist die Bedingung für den **„Uses AutoMod"-Badge** (die bisherige on_message-Filterung in [moderation.py](bot/cogs/moderation.py) zählt dafür **nicht**).
  - **Mapping (je BLOCK_MESSAGE-Action):** Banned-Words + Anti-Invite/Anti-Link → eine **KEYWORD**-Regel (`keyword_filter` + RE2-`regex_patterns`); Anti-Spam → **SPAM**-Regel; Anti-Mention (`max_mentions`) → **MENTION_SPAM**-Regel. `exempt_role_ids`/`ignored_channel_ids` → AutoMod-Exemptions. Der Bot verwaltet nur **eigene** Regeln (feste Namen „ProjectX • …"), fasst fremde AutoMod-Regeln nie an.
  - **Schema v54:** `guild_moderation_settings.automod_native` + Tabelle `guild_automod_state` (dirty/status). Jede Moderations-Speicherung → `markAutomodDirty`; db.js `getPendingAutomodSync`/`setAutomodApplied`/`getAutomodState`.
  - **Backend:** modules.js Moderation-GET liefert zusätzlich `automod` (Status), PUT validiert `automod_native` + markiert dirty. Bot-Endpoints in [bot.js](backend/routes/bot.js): `GET /api/bot/automod/pending` + `PUT /api/bot/guilds/:id/automod/applied`.
  - **Bot:** neuer Cog [automod_sync.py](bot/cogs/automod_sync.py) (@tasks.loop 90s) — reconciled create/edit/delete, meldet Status-Code (missing_permission/rule_limit/…), den das Dashboard übersetzt. `AUTOMOD_POLL_INTERVAL` in [config.py](bot/config.py), in [main.py](bot/main.py) geladen. Braucht **MANAGE_GUILD** (bereits in der Invite-Bitmask).
  - **Frontend:** [Moderation.vue](frontend/src/pages/Moderation.vue) — Toggle + „AutoMod-Badge"-Badge + Hinweis (Badge-Bedingung, Manage-Server nötig) + Live-Sync-Status/Fehler. i18n `moderation.am*` (12 Keys) in **allen 5 Sprachen** (tr/ru/pl via Sub-Agents; Parität 1882/Locale).
  - Verifiziert: Migration v54 + DB-Smoke (automod_native/pending-Config/applied-ok/error) grün, 74/74 Backend-Tests, alle Cogs kompilieren, AutoMod-Objekt-Konstruktion gegen discord.py 2.7.1 geprüft, Frontend-Build grün, i18n-Parität grün. **Hinweis:** Der Badge wird von Discord **automatisch** vergeben, sobald der Bot über genügend Server hinweg genug AutoMod-Regeln erstellt hat (kolportiert ~100, Discord veröffentlicht keine offizielle Zahl; dauert Tage–Wochen). Native AutoMod + Bot-Filter koexistieren (AutoMod blockt vor dem Senden). **⚠️ Nicht live gegen Discord getestet.**
- **Neues Bot-Profil-Modul (Schema v53, Free):** Ein Admin kann dem Bot **nur für einen Server** einen eigenen **Nickname + Avatar** geben (Discord unterstützt server-spezifische Bot-Profile).
  - **Schema v53:** Tabelle `guild_bot_profile` (`guild_id PK`, `nickname`, `avatar_url`, `dirty`, `status`, `status_message`; FK CASCADE, idempotent + Mirror). db.js: `BOT_PROFILE_DEFAULTS` + `getBotProfile`/`upsertBotProfile` (Nick-Cap 32, Avatar via `sanitizeUrlLike`, Save → `dirty=1`+`status='pending'`)/`getPendingBotProfiles`/`setBotProfileApplied`.
  - **Backend:** Cookie-Route [botprofile.js](backend/routes/botprofile.js) (`GET/PUT /api/guilds/:id/botprofile`, Audit `UPDATE_BOT_PROFILE`), Mount in [server.js](backend/server.js) (**ungegated = Free**). Bot-Endpoints in [bot.js](backend/routes/bot.js): `GET /api/bot/botprofile/pending` + `PUT /api/bot/guilds/:id/botprofile/applied`.
  - **Bot:** neuer Cog [bot_profile.py](bot/cogs/bot_profile.py) (@tasks.loop 60s) — pollt dirty-Profile, wendet `guild.me.edit(nick=…, avatar=…)` an (Avatar-Download via aiohttp, ≤10MB), meldet Status-**Code** zurück (nick_forbidden/avatar_download_failed/avatar_too_large/avatar_rate_limited/…), den das Dashboard übersetzt. Leerer Nickname → Standardname; leeres Avatar → globales Avatar. `BOT_PROFILE_POLL_INTERVAL` in [config.py](bot/config.py), in [main.py](bot/main.py) geladen.
  - **Frontend:** Seite [BotProfile.vue](frontend/src/pages/BotProfile.vue) (Nickname-Input + Avatar-URL + runde Live-Vorschau + Anwende-Status/Fehler-Anzeige). Router-Child `botprofile`, Sidebar-Link (Configuration), Overview-Card. **Invite-Bitmask `285223990` → `352332854`** (+ `CHANGE_NICKNAME`, in [Overview.vue](frontend/src/pages/Overview.vue)). i18n: Namespace `botProfile` (22 Keys inkl. Fehler-Codes) + `sidebar.linkBotProfile` + `overview.botProfile*` in **allen 5 Sprachen** (tr/ru/pl via Sub-Agents; Parität 1871/Locale).
  - Verifiziert: Migration v53 + DB-Smoke (Default/Set→dirty/pending/getPending/applied-ok/error-Status) grün, 74/74 Backend-Tests, alle Cogs kompilieren, Frontend-Build grün (BotProfile-Chunk), i18n-Parität grün. **Hinweis:** Der Bot braucht **„Nickname ändern"** für den Namen (jetzt in der Invite-Bitmask; bestehende Server ggf. neu einladen / Recht erteilen); Avatar-Änderungen sind von Discord rate-limitiert. **⚠️ Nicht live gegen Discord getestet.**
- **Admin-Rechtesystem (Schema v52):** Der Owner kann jetzt **weitere Admins** hinzufügen und pro Person granular festlegen, **welche Tabs** sie sehen und **ob sie dort nur ansehen oder auch handeln** dürfen (view/manage pro Tab).
  - **Modell:** Tabelle `admin_staff` (`user_id` PK, `permissions` JSON `{tab:'view'|'manage'}`). Der Env-Owner (`OWNER_DISCORD_ID`) hat impliziten Vollzugriff + verwaltet als **Einziger** Staff. db.js: `ADMIN_PERM_TABS` (14 Tabs, Single Source) + `getAdminStaff`/`upsertAdminStaff`/`removeAdminStaff`/`isAdminStaff`/`getAdminAccess`/`adminCan` (`manage` impliziert `view`; Owner→alles).
  - **Backend-Enforcement (2 Schichten):** (1) `requireAdmin` (Owner ODER Staff) in [session.js](backend/middleware/session.js) ersetzt `requireOwner` am Admin-Mount + legt `req.adminAccess` ab. (2) Zentraler `PERM_RULES`-Gate in [admin.js](backend/routes/admin.js) mappt jede der **47 Routen** (Method+relativer Pfad) auf Tab+Level und prüft `adminCan` — **unmapped → fail-closed für Staff** (Owner passt immer). `/staff*` = owner-only, `/access` = jeder Admin. Neue Endpoints `GET /access`, `GET/POST /staff`, `DELETE /staff/:id` (Audit `ADMIN_STAFF_UPSERT`/`_REMOVE`). `is_admin` (Owner||Staff) in `/auth/me` + `/auth/callback`.
  - **Frontend:** Router-Guard `/admin` von `requiresOwner` → `requiresAdmin` (Owner||Staff); Admin-Link in NavBar + MobileAccountSheet jetzt für `is_admin`. [Admin.vue](frontend/src/pages/Admin.vue) fetcht `/api/admin/access` → filtert die sichtbaren Tabs nach `view`-Recht (landet auf dem ersten erlaubten Tab) und **gated alle Aktions-Controls** (Sperren/Tier/Retry/Clear/Create-Panels/Delete/System-Save/Inspect-Premium …) nach `can(tab,'manage')`. Neuer **owner-only Staff-Tab** = Rechte-Matrix (pro Tab None/View/Manage-Segment) + Liste.
  - **i18n:** `admin.tabStaff` + 21 `admin.staff*`-Keys in **allen 5 Sprachen** (tr/ru/pl via Sub-Agents; Parität 1839/Locale).
  - Verifiziert: Migration v52 + DB-Smoke (Sanitize/view↔manage/Owner-all/isAdminStaff/remove) grün, **Gate-Path-Resolution** (Express-Router relativ `/users` etc.) isoliert getestet, 74/74 Backend-Tests, Frontend-Build grün, i18n-Parität grün. **Hinweis:** `is_admin` wird bei **jedem** `/auth/me` frisch berechnet (nicht im JWT gespeichert) — ein neu angelegter Staff-Admin muss die Seite nur **neu laden** (fetchMe), kein Re-Login nötig. Tab-Sichtbarkeit + Gating kommen live aus `/api/admin/access`.
- **Datum:** 2026-07-15
- **Partner-Seite (Schema v51):** Neue öffentliche Seite **`/partners`** + Owner-Admin-Tab „Partner", exakt nach dem Vorbild des Team-Systems — aber für **Partner**: entweder ein **Discord-Nutzer** (per ID) **oder ein Partner-Server** (per Einladungslink).
  - **Auflösung:** wie beim Team resolved der Bot die Anzeige. Neuer Cog [partner_resolve.py](bot/cogs/partner_resolve.py) (@tasks.loop 5min): `kind='user'` → `fetch_user` (Name/Avatar), `kind='guild'` → `fetch_invite` (Servername/Icon/Mitgliederzahl/Guild-ID — **funktioniert ohne Mitgliedschaft im Partner-Server**, genau der Sinn). In [main.py](bot/main.py) geladen.
  - **Schema v51:** Tabelle `partners` (`id` UUID, `kind ∈ {user|guild}`, `discord_id`/`guild_id`/`invite_url`, `resolved_name` (Resolved-Marker), `name`/`description`/`avatar_url` (Owner-Overrides), `member_count`, `position`; idempotent + Mirror). db.js: `getPartners`/`createPartner`/`updatePartner`/`deletePartner`/`getUnresolvedPartners`/`resolvePartner` + `PARTNER_KINDS` (Invite-Normalisierung, Name/Avatar-Fill nur wenn leer, Reset des Resolve-State bei Kind-/ID-/Invite-Wechsel).
  - **Backend:** Owner-CRUD `/api/admin/partners` (Audit `ADMIN_PARTNER_*`, 400 `partner_invalid`) in [admin.js](backend/routes/admin.js) + öffentlich `GET /api/public/partners` (Cache 60s) in [public.js](backend/routes/public.js) + Bot-Endpoints (`/api/bot/partners/unresolved`, `PUT .../resolved`) in [bot.js](backend/routes/bot.js).
  - **Frontend:** öffentliche [Partners.vue](frontend/src/pages/Partners.vue) (Route `/partners`, Footer-Link) — Karten mit Avatar/Icon, Kind-Badge (Creator/Server), Mitgliederzahl + „Server beitreten"-Button (Guild-Invite) bzw. „Besuchen"-Link (User). Admin-Tab „Partner" in [Admin.vue](frontend/src/pages/Admin.vue) (Umschalter User/Server, konditionale Felder Discord-ID vs. Einladungslink, Liste). i18n: Namespace `partners` (9) + `seo.partners*` (2) + `admin.tabPartners` + `admin.partner*` (25) in **allen 5 Sprachen** (tr/ru/pl via Sub-Agents; Parität verifiziert 1818/Locale).
  - **Avatar/Icon automatisch:** Das Avatar-Feld ist optional — bleibt es leer, füllt der Bot es automatisch (User-Avatar bzw. Server-Icon), das Admin-Formular weist per Hinweis darauf hin (`partnerAvatarHintUser/Guild`). Ein manuell gesetztes Avatar gewinnt (Fill-nur-wenn-leer); **leert** man das Feld wieder auf einem bereits aufgelösten Eintrag, resettet `updatePartner` den Resolve-State → der Bot holt Avatar/Icon neu.
  - Verifiziert: Migration v51 + DB-Smoke (User/Guild-Create, Invite-Normalisierung, Validierung, unresolved→resolve→gefüllt, Invite-Wechsel→re-resolve) grün, 74/74 Backend-Tests, Bot kompiliert (`partner_resolve.py`+`main.py`), Frontend-Build grün (Partners-Chunk), i18n-Parität grün. **Hinweis:** Auflösung greift binnen ~5 min (Poll-Intervall); für Partner-Server einen **gültigen, nicht-ablaufenden Invite** verwenden. **⚠️ Nicht live gegen Discord getestet.** Stapelt auf dem Server-Logs-Branch (v50) → nach dessen Merge mergen.
- **Datum:** 2026-07-14
- **Server-Logs-Modul massiv erweitert (Schema v50):** Das Logs-Modul deckt jetzt deutlich mehr ab — mehr Event-Typen, getrennte Log-Kanäle pro Kategorie, ein „wer war es"-Executor via Audit-Log und feinere Filter.
  - **Kategorie-Kanäle:** neben dem Standard-Kanal (`log_channel_id`) gibt es 4 optionale Kategorie-Kanäle (`member_/message_/voice_/server_log_channel_id`) — jeder fällt auf den Standard zurück, wenn leer. Der Bot routet jedes Event über `_channel_id_for(settings, category)`.
  - **7 neue Event-Typen:** `log_bulk_delete` (Massen-Löschungen), `log_boosts` (Server-Boosts via `on_member_update.premium_since`), `log_invites` (Invite create/delete), `log_threads` (Thread create/delete), `log_emojis` (Emoji/Sticker-Diff), `log_automod` (`on_automod_action`), `log_webhooks` (`on_webhooks_update`). Cog [logs.py](bot/cogs/logs.py) jetzt 23 Listener (Log-Text bleibt **EN** = Admin-Audit).
  - **Filter:** `ignored_role_ids` + `ignored_user_ids` (JSON, Events dieser Rollen/User werden übersprungen) + `ignore_bots` (Bot-Events überspringen) → `_is_actor_ignored`. Der bestehende Channel-Ignore (`log_ignored_channel_ids`) bleibt für Nachrichten-Events.
  - **Executor („wer war es"):** `show_executor` löst über das Audit-Log den verantwortlichen Moderator auf (`_find_executor` — best-effort, braucht `view_audit_log`, 12s-Fenster, Newest-First-Break). Bei Bans/Unbans/Kicks/Rollen-/Channel-/Timeout-Änderungen/Webhooks wird „Performed by" ins Embed gehängt; ein Austritt mit passendem Kick-Audit-Eintrag wird als **„Member kicked"** gelabelt.
  - **Schema v50:** 15 idempotente ALTERs auf `guild_log_settings` + Mirror. `LOG_DEFAULTS`/`getLogSettings`/`upsertLogSettings` (Snowflake-validierte Kategorie-Kanäle + JSON-Filter), [modules.js](backend/routes/modules.js)-PUT-Validierung + [bot.js](backend/routes/bot.js)-GET-Shape erweitert.
  - **Frontend** ([Logs.vue](frontend/src/pages/Logs.vue)): Kanäle-Card (Standard + 4 Kategorie-ChannelSelectors, clearbar), nach Kategorie gruppierte Event-Toggles (datengetrieben `eventGroups`), Filter-Card (ignore_bots/show_executor Toggles, RoleSelector-multi für Rollen, ID-Input+Chips für User, ChannelSelector+Chips für Channels). i18n `logs.*` um ~40 Keys erweitert in **allen 5 Sprachen** (tr/ru/pl via 3 parallele Sub-Agents; Key-Parität verifiziert 1781/Locale).
  - **Kein Intent-Change nötig** (invites/threads/emojis/webhooks/automod sind alle non-privileged, in `Intents.default()` enthalten). Für den Executor + AutoMod-Events sollte der Bot **View Audit Log** haben.
  - Verifiziert: Migration v50 + DB-Smoke (Kategorie-Kanäle, Snowflake-Filter, neue Flags, ignore/executor) grün, alle Cogs kompilieren, Frontend-Build grün, i18n-Parität 1781/Locale grün. **⚠️ Nicht live gegen Discord getestet** — nach Deploy die neuen Event-Typen + Executor einmal durchklicken.
- **Ticket-Transcripts als HTML statt TXT (kein Schema-Change):** `_maybe_transcript` in [tickets.py](bot/cogs/tickets.py) exportiert das Ticket-Transkript beim Schließen jetzt als **gestaltete `.html`-Datei** (Discord-Dark-Optik) statt Klartext-`.txt` — mit Avataren, Autor/Handle/Zeitstempel, Bild-Anhängen als `<img>` (Datei-Anhänge als Link) und Kopf (Server/Nachrichtenzahl/geschlossen-von). Neuer Helper `_render_transcript_html` + Modul-Konstante `_TRANSCRIPT_STYLE` (inline-CSS als plain String, damit die `{}` nicht f-string-escaped werden müssen). **Sicherheit:** **alle** Nachrichten-Inhalte/Namen/URLs werden `html.escape`-t (die Datei wird im Browser geöffnet → sonst HTML/Script-Injection); Attachment-URLs mit `quote=True`. Dateiname sanitisiert (`transcript-<safe>.html`). Verifiziert: kompiliert, Render-Test grün (valides Doc, `<script>` escaped, Bild-Anhang + `&`→`&amp;` in URL). **⚠️ Nicht live gegen Discord getestet.**
...
- **Datum:** 2026-07-12
- **Components V2: Thumbnail-/Section-Block (kein Schema-Change):** Der V2-Builder bekommt einen 4. Block-Typ **`section`** = Text links + **kleines Thumbnail rechts** (Discord `Section` mit `Thumbnail`-Accessory, discord.py 2.7.1). Shape `{ type:'section', content, thumbnail }` im vorhandenen `blocks[]`-JSON. `V2_BLOCK_TYPES`/`sanitizeV2Blocks` in [db.js](backend/db.js) erweitert (content-Cap 4000, Thumbnail via `sanitizeUrlLike` → erlaubt `{user.avatar}`). Bot [rich_message.py](bot/utils/rich_message.py) rendert `discord.ui.Section(TextDisplay, accessory=Thumbnail)` mit **Fallbacks**: nur-Text → TextDisplay, nur-Thumb → MediaGallery (Discord verlangt bei Section beides). Frontend: [EmbedEditor.vue](frontend/src/components/EmbedEditor.vue) neuer Block (Textarea + Thumbnail-URL + „User-Avatar"-Button; Platzhalter-Chips zielen jetzt auf Text- **und** Section-Blöcke), [DiscordMessagePreview.vue](frontend/src/components/DiscordMessagePreview.vue) rendert die Section-Optik (Text links inkl. Markdown, Thumb rechts 72px), Store [guildSettings.js](frontend/src/stores/guildSettings.js) `mergeV2Blocks` reicht `section` durch. i18n: 4 neue `embedEditor.*`-Keys (block_section/addSection/sectionTextPlaceholder/sectionThumbLabel) in **allen 5 Sprachen** (Parität 1727/Locale). Verifiziert: Bot-Smoke (Section + beide Fallbacks → valides Payload), db.js-Import, Frontend-Build grün. ⚠️ Nicht live gegen Discord getestet.
- **Embed-Vorschau rendert jetzt Markdown (kein Schema-Change):** Die Dashboard-Vorschau [DiscordMessagePreview.vue](frontend/src/components/DiscordMessagePreview.vue) zeigte `**fett**`, `#`-Header etc. bisher als Rohtext — der Bot rendert Markdown ja nativ, nur die Preview hinkte hinterher. Neuer `renderMarkdown(raw, allowHeaders)` (+ `inlineMd`): **fett**/*kursiv*/__unterstrichen__/~~durchgestrichen~~/`code`/||spoiler||/[label](url)/`>` Zitat/`-` Liste, plus `#`/`##`/`###` Header. **Header nur wo Discord sie auch rendert** — Nachrichten-Content + V2-TextDisplays (`allowHeaders=true`), **nicht** in klassischen Embed-Descriptions (`allowHeaders=false`, dort bleibt `#` literal). Input wird zuerst HTML-escaped (kein Injection über `v-html`), Masked-Links nur `https?://`. Scoped-CSS für `.md-h1..3`/`.md-code`/`.md-link`/`.md-spoiler`/`.md-quote`/`.md-li` in allen 3 Text-Containern. Reine Vorschau-Änderung (Bot unberührt). Frontend-Build grün.
...
- **Changelog-Publisher (Schema v47, Owner-Admin):** Der Owner schreibt Release-Notes im neuen Admin-Tab **„Changelog"**; veröffentlichte Einträge erscheinen auf einer öffentlichen Seite **`/changelog`** (kein Auth, Footer-Link).
  ...
  - **Backend:** Owner-CRUD in [admin.js](backend/routes/admin.js) (`/api/admin/changelog`, Audit `ADMIN_CHANGELOG_*`, 400 `title_required`) + öffentlich `GET /api/public/changelog` (Cache 60s) in [public.js](backend/routes/public.js).
  - **Frontend:** öffentliche Seite [Changelog.vue](frontend/src/pages/Changelog.vue) (Route `/changelog`, Footer-Link) — Karten mit Version-Badge/Datum/Body; Body via `white-space: pre-wrap` (**kein** Markdown-Renderer, **kein** `v-html`). Admin-Tab „Changelog" in [Admin.vue](frontend/src/pages/Admin.vue) (Erstellen/Bearbeiten/Veröffentlichen-Umschalten/Löschen; Auto-Refresh-Guard um `changelog + clFormDirty()` erweitert). i18n: Namespace `changelog` + `footer.changelog` + 23 `admin.cl*`/`admin.tabChangelog`-Keys (28 gesamt) in **allen 5 Sprachen** (TR/RU/PL via Sub-Agents; Parität verifiziert).
  - Verifiziert: Migration v47 + DB-Smoke (Create/List/Published-Filter/Update-Publish/Validierung/Delete) grün, Backend-Syntax OK, Frontend-Build grün, i18n-Parität grün.
- **Datum:** 2026-07-06
- **`/ticketpanel` als Slash-Command (kein Schema-Change):** Der Prefix-Befehl `!ticketpanel` ist jetzt der Slash-Command **`/ticketpanel`** ([tickets.py](bot/cogs/tickets.py), `app_commands.command` im Cog, `default_permissions(administrator=True)` + Runtime-Admin-Check, ephemerale Antworten via defer/followup). `BUILTIN_COMMANDS` in [db.js](backend/db.js): Eintrag `ticketpanel` von `prefix` auf `slash` umgestellt (Key bleibt → Command-Manager-Toggle greift weiter). Neuer i18n-Key `ticket.adminOnly` in allen 5 Sprachen. Frontend: `tickets.usageNote` (Hinweis „nach dem Speichern /ticketpanel ausführen") + `customCommands.prefixHint`-Beispiel (`!suggest` statt `!ticketpanel`) in allen 5 Sprachen aktualisiert. Verifiziert: Cog kompiliert, Bot-i18n-Parität + Frontend-Parität + Build grün. **Hinweis:** `/ticketpanel` erscheint erst nach Bot-Neustart (`tree.sync()`). Die Ticket-Zeilen ([TicketCategoryRow.vue](frontend/src/components/TicketCategoryRow.vue)) sind seit demselben Tag zusätzlich einklappbar.
- **Ticket-Kategorien einklappbar (kein Schema-Change):** [TicketCategoryRow.vue](frontend/src/components/TicketCategoryRow.vue) bekommt den Einklapp-Mechanismus (Caret + klickbarer Header, Body in `v-show`) wie StatsCounterRow/RoleMenuRow — gespeicherte Kategorien starten eingeklappt, Drafts offen. Neuer i18n-Key `tickets.catToggleDetails` in allen 5 Sprachen.
- **Temp-Voice-Panel: klare Berechtigungs-Fehler statt „Missing Access" (kein Schema-Change):** Die Panel-Buttons bearbeiten den Kanal (Lock/Unlock/Hide/Show + Invite/Kick → Channel-Overwrites brauchen **Rollen verwalten**; Limit/Rename → **Kanäle verwalten**). Fehlte eins, gab Discord `50001 Missing Access` zurück, was der Bot als generische bzw. rohe Meldung zeigte. Jetzt fängt [tempvoice.py](bot/cogs/tempvoice.py) `discord.Forbidden` gezielt ab (`on_interaction`, LimitModal/RenameModal, `_apply_invite`/`_apply_kick`) und antwortet mit einer klaren, lokalisierten Meldung, die die **fehlende Berechtigung benennt** (`tv.missingPermRoles`/`tv.missingPermChannels`, neu in allen 5 Sprachen). **Eigentliche Behebung serverseitig:** dem Bot **Rollen verwalten** + **Kanäle verwalten** geben (bzw. neu mit dem vollen Invite einladen) und sicherstellen, dass die Temp-Voice-Kategorie diese Rechte nicht entzieht. Verifiziert: kompiliert, Bot-i18n-Parität grün.
...
- **Dashboard-Auto-Refresh (kein Schema-Change):** Das Dashboard hält seine Daten jetzt **automatisch aktuell** — kein manueller Reload mehr nötig, egal ob Live-Zähler, Bestenlisten, Server-/Bot-Status, Job-Queue oder Einstellungen.
  ...
  - **Verdrahtung:** in praktisch allen Modul-Seiten eingehängt. Muster: Settings-Seiten `{isDirty: () => dirty.value}`; Store-basierte (Welcome/Leave) forcieren `store.loadFull`; Listen-Seiten mit Inline-Editor guarden auf offenen Draft (`!!draftRow.value`/`editorOpen`) **und** die Zeilen-Komponenten ([SocialSubscriptionRow](frontend/src/components/SocialSubscriptionRow.vue)/[RoleMenuRow](frontend/src/components/RoleMenuRow.vue)/[CustomCommandRow](frontend/src/components/CustomCommandRow.vue)/[ScheduledMessageRow](frontend/src/components/ScheduledMessageRow.vue)/[StatsCounterRow](frontend/src/components/StatsCounterRow.vue)/[TicketCategoryRow](frontend/src/components/TicketCategoryRow.vue)) re-hydratisieren **nicht** während sie dirty sind; read-only Ansichten (Overview/Polls/Premium/Servers/Team/Leaderboards) ohne Guard. [Backup.vue](frontend/src/pages/Backup.vue) refresht zusätzlich zum 4s-Job-Poller (Guard: offene Modals). [Admin.vue](frontend/src/pages/Admin.vue) refresht den aktiven Tab (Guard: `system`-Tab + offene Modals, da der System-Tab editierbare Formulare lädt). [Minecraft.vue](frontend/src/pages/Minecraft.vue) ist ebenfalls verdrahtet (Guard: offener Draft).
  - **Music.vue** behält sein eigenes 4s-Live-Polling; Auto-Refresh nur für die Settings.
  - **Umsetzung:** Composable + 5 Referenz-Seiten selbst, Rest über 4 parallele Sub-Agents (disjunkte Dateien). Verifiziert: Frontend-Build grün, alle Guard-Refs vorhanden.
...
- **Verification-Modul erweitert — Embed-Designer + Auto-Update (Schema v45):** Das Verify-Panel kann jetzt ein **voll gestaltetes Embed** sein (statt nur Plain-Nachricht), und das **gepostete Panel aktualisiert sich automatisch**, wenn man es im Dashboard ändert (Muster wie beim Rollen-Menü-Live-Update).
  ...
  - **Bot** ([verification.py](bot/cogs/verification.py)): neuer 60s-`post_loop` → `GET /api/bot/verification/pending` → postet das Panel automatisch (enabled + Channel + Rolle gesetzt) bzw. **editiert es in-place** bei dirty; `build_panel_embed` (Plain ODER Embed, `{guild}`-Platzhalter). `/verifypanel` nutzt dieselbe Render-Logik (manueller (Re)Post). Backend-Endpoint `GET /api/bot/verification/pending` in [bot.js](backend/routes/bot.js).
  - **Frontend** ([Verification.vue](frontend/src/pages/Verification.vue)): Modus-Umschalter Plain/Embed + wiederverwendeter `EmbedEditor` + **Live-Vorschau** (DiscordMessagePreview mit Verify-Button-Mock). i18n: 9 neue `verification.*`-Keys + aktualisierte `usageNote` in **allen 5 Sprachen** (TR/RU/PL via Sub-Agents).
  - Verifiziert: Migration v45 + DB-Smoke (Embed-Persistenz/`dirty`-Flow: unposted→post→edit→dirty/in-place→re-render→0, disabled) grün, Cog kompiliert + importiert gegen discord.py 2.7.1 (post_loop + verifypanel-app_command), Frontend-Build grün, i18n-Parität grün. **Hinweis:** Auto-Post/Update braucht einen gesetzten **Channel + Rolle**; Änderungen greifen binnen ~1 Min (Poll-Intervall).
...
- **Team-/Credits-Seite (Schema v44):** Neue öffentliche Seite [`/team`](frontend/src/pages/Team.vue) (kein Auth) zeigt, wer am Projekt mitwirkt — Karten mit Avatar (oder Initialen-Fallback), Rolle, Bio und Social-Icons. Der **Owner verwaltet die Mitglieder im Admin-Bereich** (neuer Tab „Team"): Hinzufügen per **Discord-ID/Tag** + **Socials** (github/twitter/youtube/twitch/instagram/tiktok/website), Bearbeiten/Löschen/Reihenfolge.
  ...
  - **Backend:** Admin-CRUD `/api/admin/team` (Owner, Audit `ADMIN_TEAM_*`) + öffentlich `GET /api/public/team` (Cache 60s) in [public.js](backend/routes/public.js).
  - **Frontend:** öffentliche [Team.vue](frontend/src/pages/Team.vue) (Route `/team`, Footer-Link) — rendert Social-Links **nur** wenn `https?://` (kein `javascript:`); Admin-Tab „Team" in [Admin.vue](frontend/src/pages/Admin.vue) (Formular + Liste). i18n: Namespace `team` (4 Keys) + `admin.tabTeam` + 22 `admin.team*`-Keys in **allen 5 Sprachen** (TR/RU/PL via Sub-Agents).
  - **Bot-Auflösung per Discord-ID (gleicher v44-Umbau):** Ein Mitglied kann jetzt **nur mit einer Discord-ID** angelegt werden (Name optional) — der Bot-Cog [team_resolve.py](bot/cogs/team_resolve.py) (@tasks.loop 5min) pollt `GET /api/bot/team/unresolved`, holt per `fetch_user` **Name (`global_name`) + Avatar + Username** und schreibt sie via `PUT /api/bot/team/:id/resolved` zurück. Füllt Name/Avatar **nur wenn leer** (manuell gesetzte Werte gewinnen), setzt `discord_username` als Resolved-Marker (→ keine Endlos-Retries; ungültige ID → `unknown`). Schema: `team_members.discord_username` **in der v44-Tabelle** ergänzt (idempotenter ALTER, kein v45). `createTeamMember`/`updateTeamMember` verlangen jetzt Name **ODER** discord_id; neue Helfer `getUnresolvedTeamMembers`/`resolveTeamMember`. Admin-Formular: Name optional bei ID, Hinweis + `@username` in der Liste. i18n: `admin.teamNameOrId`/`teamResolveHint` (5 Sprachen).
  - Verifiziert: Migration v44 + DB-Smoke (Create/List/Update/Delete/Whitelist-Schutz/Validation **+ ID-only→unresolved→resolve→gefüllt, manuell nicht überschrieben**) grün, Backend-Syntax OK, Bot kompiliert (`team_resolve.py`+`main.py`), Frontend-Build grün, i18n-Parität grün. **Hinweis:** Auflösung greift binnen ~5 min (Poll-Intervall); Avatar-Fallback via `users`-Join bleibt zusätzlich für Dashboard-User.
- **Datum:** 2026-07-01
...
- **Neues Music-Modul (Schema v43, Pro) — Lavalink-Voice-Wiedergabe, OHNE YouTube:** Neues 35. Modul: Musik in Sprachkanälen via Lavalink (wavelink 3.x). Quellen: **SoundCloud / Bandcamp / Twitch / Direkt-+Radio-Streams — kein YouTube** (Lavalink-seitig nicht installiert + im Cog defensiv geblockt).
  ...
  - **Slash-Commands:** `/play /skip /stop /pause /resume /queue /nowplaying /volume /loop /shuffle` (10 neue `BUILTIN_COMMANDS`). DJ-Rollen-Gate (skip/stop/control nur DJ/Admin), Quellen-Whitelist, max. Queue, Default-Volume, Auto-Disconnect bei allein/leer.
  - **Schema v43:** 3 Tabellen `guild_music_settings` / `guild_music_state` (Bot-Live-Snapshot) / `guild_music_commands` (Dashboard-Steuerungs-Queue). db.js-Helfer (`get|upsertMusicSettings`, `get|upsertMusicState`, `enqueueMusicCommand`/`getAllMusicCommands`/`deleteMusicCommand`) + Konstanten. `MODULE_TIERS.music='pro'`, `PREMIUM_BOT_GATES`-Eintrag `/settings/music$`.
  - **Backend:** neue Cookie-Route [music.js](backend/routes/music.js) (`/music` Settings GET/PUT, `/music/state` GET Live, `/music/control` POST Steuerung) + Pro-Gate-Mount in [server.js](backend/server.js); Bot-Endpoints in [bot.js](backend/routes/bot.js) (`GET /settings/music`, `PUT /music/state`, `GET+DELETE /music/commands`).
  - **Frontend:** Seite [Music.vue](frontend/src/pages/Music.vue) — **Live-Player** (Now-Playing + Queue, pollt `/music/state` alle 4s) mit **Dashboard-Steuerung** (Play/Pause/Skip/Stop/Shuffle/Loop/Volume/Remove/Clear via `/music/control`-Jobs) + **Settings** (DJ-Rolle/Default-Volume/Auto-Disconnect/max. Queue/Quellen-Toggles). Router-Child `music`, Sidebar-Link (Engagement), Overview-Card (35), PremiumLock automatisch. i18n: `sidebar.linkMusic` + `overview.music*` + Namespace `music` (35 Keys) in **allen 5 Sprachen** + 28 Bot-i18n-Keys (`music.*`, 5 Sprachen).
  - **⚠️ Nicht live testbar hier:** Audio-Wiedergabe braucht laufendes Lavalink + Discord-Voice. Verifiziert wurde: Migration v43 + DB-Smoke (Settings/State/Command-Queue) grün, Bot kompiliert (`music.py`), Backend-Syntax OK, Frontend-Build grün, i18n-Parität (Bot 286 / Frontend). **Vor Nutzung:** Lavalink-Container starten, Bot-`LAVALINK_*`-Env setzen, `pip install wavelink`, Bot neu starten (Slash-Sync).
- **Giveaway-Modul überarbeitet — Dashboard-Erstellung + Slash-Commands + Teilnahme-Voraussetzungen (Schema v42):** Das Giveaway-Modul kann jetzt im Dashboard erstellt werden, läuft über Slash-Commands statt Prefix, und unterstützt Teilnahme-Voraussetzungen.
  - **Schema v42:** 6 idempotente ALTERs auf `guild_giveaways` (`host_id`, `description`, `required_role_ids` JSON, `min_account_age_days`, `min_member_days`, `min_level`) + Mirror. db.js: `shapeGiveaway`/`createGiveaway` erweitert (Requirements + `host_id`/`message_id`, `ends_at` jetzt Pflicht), neue Helfer `getPendingGiveaways` (Dashboard-erstellt, noch nicht gepostet) + `endGiveawayNow`, Sanitizer `sanitizeRoleIdList`/`parseRoleIdColumn`.
  - **Slash statt Prefix:** Cog [giveaways.py](bot/cogs/giveaways.py) komplett neu — `app_commands.Group` `/giveaway` mit `start` (prize/winners/duration + optional required_role/min_account_age/min_member_age/min_level/description), `reroll`, `end` (alle `default_permissions=manage_guild` + Runtime-Check). Prefix `!gstart`/`!greroll` entfernt. `BUILTIN_COMMANDS` in [db.js](backend/db.js) entsprechend von 2 Prefix- auf 3 Slash-Einträge umgestellt (`giveaway start|reroll|end`).
  - **Voraussetzungen (Bot prüft beim Button-Klick):** benötigte Rolle(n) (any-of), Mindest-Kontoalter, Mindest-Serverzugehörigkeit (beide aus `member.created_at`/`joined_at`), Mindest-Level (via neuem `GET /api/bot/guilds/:id/leveling/user/:id`). Bei Nichterfüllung ephemerale Begründung, sonst Eintrag. Embed zeigt Voraussetzungen + Host.
  - **Dashboard-Erstellung:** neue Cookie-`POST /api/guilds/:id/giveaways` (duration_seconds → ends_at, Audit `GIVEAWAY_CREATE`); das Giveaway hat kein `message_id` → der Bot postet es per neuem `_post_pending`-Loop (`GET /api/bot/giveaways/pending`). [Giveaways.vue](frontend/src/pages/Giveaways.vue) bekommt ein Erstell-Formular (Preis/Gewinner/Dauer+Einheit/Channel/Beschreibung/Requirements) + Status-Badge „Ausstehend" + Requirement-Chips in der Liste.
  - **i18n:** 13 neue Bot-i18n-Keys (`giveaway.*`, alle 5 Sprachen, Parität 258/Sprache) + 28 neue Frontend-Keys im `giveaways`-Namespace (5 Sprachen, Parität 1526/Locale; TR/RU/PL via 3 parallele Sub-Agents).
  - Verifiziert: Migration v42 + DB-Smoke (Create mit Requirements/pending/post/shape/end-early/Validation/Katalog) grün, Bot kompiliert (`giveaways.py`+`bot_i18n.py`), Backend-Syntax OK, Frontend-Build grün, i18n-Parität Bot 258 + Frontend 1526 grün. **Hinweis:** Slash-Commands erscheinen erst nach Bot-Neustart (`tree.sync()` in setup_hook); Dashboard-Giveaways werden binnen ~30s gepostet.
- **Datum:** 2026-06-30
- **Rollen-Menü aktualisiert sich nach Dashboard-Bearbeitung (Schema v41):** Bisher hat eine Inhalts-/Options-/Embed-Änderung an einem bereits **geposteten** Rollen-Menü die Nachricht im Channel **nicht** aktualisiert — der Bot postete nur noch nie gepostete Menüs (`getPendingRoleMenus` filterte `message_id IS NULL`), und `updateRoleMenu` löschte `message_id` nur bei **Channel-Wechsel**. Inhaltsänderungen im selben Channel blieben damit stumm.
  - **Fix (Schema v41 = `guild_role_menus.dirty`):** Jede Dashboard-Bearbeitung (`updateRoleMenu`) setzt `dirty = 1`. `getPendingRoleMenus()` liefert jetzt unposted **ODER** dirty-Menüs. Der [rolemenus.py](bot/cogs/rolemenus.py)-Cog (`_post_menu`) editiert bei vorhandener `message_id` die **bestehende Nachricht in-place** (`channel.fetch_message` + `message.edit`; Fallback Repost, wenn die Nachricht gelöscht wurde) statt zu duplizieren, und `setRoleMenuMessage` setzt `dirty = 0` zurück. Channel-Wechsel löscht weiterhin `message_id` (frischer Post im neuen Channel; die alte Nachricht im alten Channel bleibt verwaist — unverändertes Vorverhalten).
  - **Hinweis Verzögerung:** Der Cog pollt alle 60s (`POLL_SECONDS`), die Aktualisierung erscheint also bis zu eine Minute nach dem Speichern.
  - Verifiziert: Migration v41 + DB-Smoke (Create→pending, Post→0, Edit→dirty/pending mit erhaltener message_id, Re-render→0) grün, `rolemenus.py` kompiliert, Backend-Syntax (db.js/migrations.js) OK.
- **Datum:** 2026-06-29
...
- **Backup-Restore: Pre-Flight-Permission-Check (kein Schema-Change):** Ein Restore in [server_backup.py](bot/cogs/server_backup.py) prüft jetzt **vor** dem ersten Discord-Call, ob der Bot die für die gewählten Teile nötigen Rechte hat, statt pro Element soft-zu-failen.
  ...
  - Modul-Docstring (GOTCHAS) ergänzt. Verifiziert: `py_compile` grün. **Hinweis an Nutzer:** Der eigentliche Auslöser ist die Discord-Rollen-Config — der Bot braucht „Rollen verwalten" + „Kanäle verwalten" (+ „Server verwalten" für Name/Icon) **und** seine Rolle muss in der Server-Rollenliste hoch genug stehen; am einfachsten via Administrator-Recht.
- **Admin-Marketplace: Vorlagen hochladen + exportieren (kein Schema-Change):** Der Owner-Admin hat jetzt einen eigenen **Marketplace-Tab** ([Admin.vue](frontend/src/pages/Admin.vue)) zum Verwalten der Vorlagen — bisher gab es nur den Publish-Button je Snapshot in [Backup.vue](frontend/src/pages/Backup.vue) und Backend-Endpoints ohne UI.
  - **Upload:** JSON-Datei hochladen → erscheint sofort im Marketplace. Neuer Endpoint `POST /api/admin/marketplace/upload` (`{ name?, description?, category?, guild_name?, guild_icon_url?, data }`, `source_guild_id = null`, validiert Rollen/Kanäle → 400 `invalid_template`/`empty_template`, Audit `MARKETPLACE_UPLOAD`). Akzeptiert sowohl einen **rohen Snapshot** `{server,roles,channels}` als auch eine zuvor **exportierte Datei** (Blob unter `data` verschachtelt). Frontend parst die Datei clientseitig (FileReader), zeigt erkannte Kanal-/Rollen-Anzahl und füllt Name/Beschreibung/Kategorie vor.
  - **Export:** je Vorlage Download als re-importierbare JSON via `GET /api/admin/marketplace/:id/export` (voller `data`-Blob, `Content-Disposition: attachment`, Audit `MARKETPLACE_EXPORT`; clientseitig per Blob heruntergeladen, analog CSV-Export). Export→Upload round-trippt.
  - **Verwaltung:** der Tab listet alle Vorlagen (Status/Uses/Counts) mit Status-Select (approved/pending/rejected), Export- und Löschen-Aktion (nutzt die vorhandenen `PUT /:id/status` / `DELETE /:id`). `getMarketplaceTemplate` in [admin.js](backend/routes/admin.js) ergänzt importiert; keine DB-/Schema-Änderung (`createMarketplaceTemplate` akzeptierte `sourceGuildId=null` bereits).
  - **i18n:** neuer `admin.tabMarketplace` + 22 `admin.mp*`-Keys in **allen 5 Sprachen** (en/de selbst, tr/ru/pl via Sub-Agents; Key-Parität verifiziert).
  - Verifiziert: admin.js + Admin.vue Syntax OK, Frontend-Build grün, i18n-Parität grün.
...
- **Admin-Bereich → Betrieb & Monitoring (Schema v37):** Drei neue Owner-Admin-Funktionen.
  ...
  - **Fehler-/Exception-Viewer** — neue Tabelle `error_log` (Migration v37, idempotent + Mirror). Bot meldet via `POST /api/bot/errors` (`report_error` in [main.py](bot/main.py): `on_command_error` + globaler `on_error`), Backend via globalem Error-Handler in [server.js](backend/server.js). Admin-Tab „Errors" mit Filter (Quelle/Stufe) + Leeren. `logError`/`getErrorLog`/`clearErrorLog` in [db.js](backend/db.js) (Retention 2000). Endpoints `GET/DELETE /api/admin/errors` (Audit `ADMIN_CLEAR_ERRORS`).
  - **Job-Queue-Monitor** — Admin-Tab „Jobs" listet alle `guild_backup_jobs` über alle Guilds (mit `guild_name`), Filter nach Status, „Retry" für fehlgeschlagene (→ `pending`). `getAllBackupJobs`/`retryBackupJob` in [db.js](backend/db.js). Endpoints `GET /api/admin/jobs` + `POST /api/admin/jobs/:id/retry` (Audit `ADMIN_RETRY_JOB`).
  - **Frontend:** [Admin.vue](frontend/src/pages/Admin.vue) um 3 Tabs (health/jobs/errors) + Toolbar-Filter + `.job-status`-Badges erweitert. i18n: ~24 neue `admin.*`-Keys in **allen 5 Sprachen** (Parität 1403/Locale, TR/RU/PL via Sub-Agents).
  - Verifiziert: Migration v37 + DB-Smoke (error_log Insert/Filter/Clear, Jobs-Global-Liste/Retry, Health-Detail) grün, Backend-Syntax OK, Bot kompiliert, Frontend-Build grün, i18n-Parität 1403/Locale.
- **Datum:** 2026-06-24
- **Allgemein-Modul (Schema v36, Free):** Neues 34. Modul — eine pro-Server Seite `/dashboard/:guild_id/general` für **allgemeine Dashboard-Einstellungen**: **Sprache** (en/de/tr/ru/pl), **Zeitzone** (kuratierte IANA-Liste), **Standard-Embed-Farbe** (Hex-Picker), **Dashboard-Theme** (dark|light). Kein `enabled`-Toggle (immer aktiv).
  - **Schema:** Migration v36 = Tabelle `guild_general_settings` (`guild_id PK` + `language`/`timezone`/`embed_color`/`dashboard_theme`; idempotent + Mirror in `initializeDatabase()`). `GENERAL_LANGUAGES`/`GENERAL_THEMES`/`GENERAL_TIMEZONES`/`GENERAL_DEFAULTS` + `getGeneralSettings`/`upsertGeneralSettings` (validieren/fallbacken) in [db.js](backend/db.js). `MODULE_TIERS.general='free'`, in `MODULE_DEFAULTS`.
  - **Backend:** neue Cookie-Route [general.js](backend/routes/general.js) (`GET/PUT /api/guilds/:id/general`, Audit `UPDATE_GENERAL_SETTINGS`), Mount in [server.js](backend/server.js) (ohne Premium-Gate), Bot-GET `/api/bot/guilds/:id/settings/general` in [bot.js](backend/routes/bot.js).
  - **Frontend:** Seite [General.vue](frontend/src/pages/General.vue) (Sprache/Zeitzone-Dropdowns, Farb-Picker mit Vorschau, Theme-Swatches dark|light). Theme-Anwendung über neuen Helper [useDashboardTheme.js](frontend/src/composables/useDashboardTheme.js) (`applyDashboardTheme` setzt `data-app-theme` auf `<html>` + spiegelt nach localStorage; `initDashboardTheme()` in [main.js](frontend/src/main.js) wendet es früh an → kein Flash). Light-Theme-Token-Override (`:root[data-app-theme="light"]`) in [tokens.css](frontend/src/styles/tokens.css). Router-Child `general`, Sidebar-Link (Configuration-Gruppe, nach Übersicht), Overview-Card (34, zeigt aktive Sprache). i18n: `sidebar.linkGeneral` + `overview.generalTitle/generalDesc` + Namespace `general` (15 Keys) in **allen 5 Sprachen** (tr/ru/pl via 3 parallele Sub-Agents; Key-Parität verifiziert 1380/Locale).
  - **Bot-Konsum — Embed-Farbe (Folge-Iteration):** Die `embed_color` wird jetzt im Bot konsumiert. Neuer gecachter Helfer [general_config.py](bot/utils/general_config.py) (`get_embed_color(backend_url, api_key, guild_id, fallback)` → `discord.Color`, 60s-Cache, liest `/api/bot/guilds/:id/settings/general`). Eingehängt in **10 Cogs** an den **Marken-/Neutral-Embeds**: [suggestions](bot/cogs/suggestions.py)/[verification](bot/cogs/verification.py)/[polls](bot/cogs/polls.py)/[giveaways](bot/cogs/giveaways.py)/[tempvoice](bot/cogs/tempvoice.py) (Panel + Owner-Transfer-Notice)/[economy](bot/cogs/economy.py) (rich/shop)/[applications](bot/cogs/applications.py) (Panel + Review-Embed)/[rolemenus](bot/cogs/rolemenus.py) (Auto-Listen-Embed)/[starboard](bot/cogs/starboard.py)/[slash_utils](bot/cogs/slash_utils.py) (/serverinfo). Jeweils mit der alten Konstante als `fallback` → unverändertes Aussehen bei unerreichbarem Backend. **Bewusst NICHT überschrieben** (semantisch/eigene Farbe): Logs-Event-Farben, Moderation/Antiraid-Alerts (rot), Accept/Deny (grün/rot), `member.color` (/userinfo, /avatar), Plattform-Farben (social), User-konfigurierte Embeds (welcome/leave/tickets-Panel+Welcome, rolemenus-`use_embed`), Games (eigene Theme-Identität).
  - **Bot-Konsum — Sprache (komplett für member-sichtbare Strings):** Die `language` wird im Bot konsumiert. Geteilter i18n-Helfer [bot_i18n.py](bot/utils/bot_i18n.py): `t(lang, key, **kwargs)` über eine namespaced `STRINGS`-Tabelle (en/de/tr/ru/pl, **245 Keys**, `suggest.`/`verify.`/`poll.`/`giveaway.`/`eco.`/`app.`/`ticket.`/`mod.`/`raid.`/`bday.`/`count.`/`rm.`/`slash.`/`tv.`) auf Basis von `game_i18n.make_translator`; `lang_for(backend_url, api_key, guild_id)` löst die Sprache via `general_config.get_language` (60s-Cache) auf. **Verdrahtet in 14 Cogs:** suggestions/verification/polls/giveaways/economy/applications/tickets/[moderation](bot/cogs/moderation.py) (Filter-DMs + Anti-Spam-DM; Reason als Key → lokalisiert für DM, EN fürs Audit-Log)/[antiraid](bot/cogs/antiraid.py) (beide Alerts + Author)/[birthday](bot/cogs/birthday.py) (`!birthday`-Antworten)/[counting](bot/cogs/counting.py) (Reset-Meldungen)/[rolemenus](bot/cogs/rolemenus.py) (Auto-Liste/Platzhalter/Toggle-Antworten)/[slash_utils](bot/cogs/slash_utils.py) (/ping/userinfo/serverinfo/avatar inkl. Feld-Labels)/[tempvoice](bot/cogs/tempvoice.py) (alle 9 Panel-Buttons, Embed, Limit-/Rename-Modal, alle Interaktions-Antworten). View-Builder/Modals nehmen einen `lang`-Parameter; Lookup fällt bei fehlendem Key/Sprache auf EN zurück → nie ein Crash. TR/RU/PL via je 3 parallele Sub-Agents übersetzt (Key- + Platzhalter-Parität verifiziert).
  - **Bewusst NICHT übersetzt:** **logs** (Server-Audit-Trail — viele Event-Embed-Labels, rein Admin-facing → bleibt EN), user-authored Templates (welcome/leave/custom-commands/scheduled/birthday-announce/leveling-announce/social/invite — sind Nutzertext, kein Bot-String), und reine Event-Cogs ohne user-sichtbaren Text (autorole/reaction_roles/starboard/stats/guild_sync/presence). Games haben ihre eigene `games_language`. **Zeitzone** — Discord lokalisiert `<t:timestamp:…>` bereits **pro Betrachter** (besser als eine erzwungene Guild-TZ); `get_timezone` steht bereit, falls künftig Text-Zeiten gerendert werden.
  - Verifiziert: Migration v36 sauber, DB-Smoke grün, Backend-Syntax OK, Frontend-Build grün, i18n-Parität Frontend 1380/Locale; **Bot-i18n 245 Keys × 5 Sprachen mit Key- + Platzhalter-Parität grün** (Format-Smoke aller Multi-Platzhalter-Keys grün), gesamter Bot kompiliert (`compileall`), Farb-Parsing-Unit-Test grün.
- **Datum:** 2026-06-23
...
- **Template-Marketplace (Schema v34):** Erweiterung des Backup-Moduls — der **System-Owner** kann Server-Snapshots als öffentliche **Marketplace-Vorlagen** veröffentlichen; **jede Pro-Guild** kann sie anwenden. Beim „Vorlage anwenden" zeigt das Modal jetzt einen Umschalter **„Meine Server"** (eigene Snapshots) ↔ **„Marketplace"** (veröffentlichte Vorlagen).
  ...
  - **Backend:** Owner-only in [admin.js](backend/routes/admin.js) (`POST /marketplace/publish` — strippt Member-Overwrites zur Privatsphäre, `GET /marketplace`, `PUT /marketplace/:id/status`, `DELETE /marketplace/:id`); Pro-Browse/Apply in [backup.js](backend/routes/backup.js) (`GET /marketplace`, `GET /marketplace/:id`, `POST /apply-marketplace`). DB-Helfer + `MARKETPLACE_STATUSES` in [db.js](backend/db.js).
  - **Frontend:** [Backup.vue](frontend/src/pages/Backup.vue) — Tab-Umschalter im Apply-Modal (Marketplace-Dropdown + Beschreibung, geteilte Modus-/Teil-Auswahl), Owner-Publish-Button je Snapshot + Publish-Modal (Name/Beschreibung/Kategorie), Owner-Remove im Marketplace-Tab. i18n: 18 neue `backup.*`-Keys (`marketplace*`/`publish*`/`ownServersTab`/…) in allen 5 Sprachen (Key-Parität 1350/Locale).
  - Verifiziert: Migration v34 sauber, DB-Smoke (Publish/Member-Overwrite-Strip/Browse/Apply-Resolve-über-Join/uses/Kategorie-Filter/Delete) grün, Backend-Syntax OK, Frontend-Build grün, i18n-Parität 1350/Locale.
