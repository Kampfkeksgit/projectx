...
- **Command-Manager im Custom-Commands-Modul (Schema v19):** Das Modul listet jetzt **alle** eingebauten Befehle aller Module auf und macht sie pro Server konfigurierbar.
  - **Katalog:** `BUILTIN_COMMANDS` in [db.js](backend/db.js) ist die Single Source (15 Befehle, `key` = qualified command name, `type ∈ {prefix|slash}`, `module`, `usage`, `description`). Frontend gruppiert nach Modul und zeigt pro Befehl einen An/Aus-Toggle + Typ-Badge.
