...
- **Team/Credits:** `GET /api/admin/team` → `{ members }`; `POST /api/admin/team` body `{ name, role?, discord_id?, avatar_url?, bio?, position?, socials? }` → `{ member }` (400 `name_required`, Audit `ADMIN_TEAM_CREATE`); `PUT /api/admin/team/:id` (404 `not_found`, Audit `ADMIN_TEAM_UPDATE`); `DELETE /api/admin/team/:id` (Audit `ADMIN_TEAM_DELETE`). Öffentlich: `GET /api/public/team` → `{ members: [{ id, discord_id, name, role, avatar_url, bio, socials, position }] }` (Cache 60s, von [Team.vue](frontend/src/pages/Team.vue) geladen). `socials` nur whitelisted Keys; Frontend rendert nur `https?://`-Werte als Link.
...
- `team_members` — Team-/Credits-Einträge für die öffentliche `/team`-Seite (`id` UUID, `discord_id` optional, `name`, `role`, `avatar_url`, `bio`, `socials` JSON `{github|twitter|youtube|twitch|instagram|tiktok|website}`, `position`). Owner-verwaltet (Admin → Team); `getTeamMembers` joint `users` für einen Avatar-Fallback über `discord_id`.
...
- **Team-/Credits-Seite (Schema v44):** Neue öffentliche Seite [`/team`](frontend/src/pages/Team.vue) (kein Auth) zeigt, wer am Projekt mitwirkt — Karten mit Avatar (oder Initialen-Fallback), Rolle, Bio und Social-Icons. Der **Owner verwaltet die Mitglieder im Admin-Bereich** (neuer Tab „Team"): Hinzufügen per **Discord-ID/Tag** + **Socials** (github/twitter/youtube/twitch/instagram/tiktok/website), Bearbeiten/Löschen/Reihenfolge.
  ...
  - Verifiziert: Migration v44 + DB-Smoke (Create/List/Update/Delete/Whitelist-Schutz/Validation) grün, Backend-Syntax OK, Frontend-Build grün, i18n-Parität grün. **Hinweis:** Avatar-Fallback greift nur, wenn die Discord-ID zu einem Dashboard-User passt; sonst Initialen. Keine Live-Auflösung von Discord-Tags (Architektur: Backend kann den Bot nicht synchron fragen) — ID + optional Avatar-URL eintragen.
- **Datum:** 2026-07-01
