import express from 'express'
import { getSuggestionSettings, upsertSuggestionSettings, logAuditAction } from '../db.js'
import { requireGuildAccess } from '../middleware/auth.js'
import { requireSession } from '../middleware/session.js'

const router = express.Router({ mergeParams: true })

router.get('/', requireSession, requireGuildAccess, async (req, res) => {
  try {
    const settings = await getSuggestionSettings(req.params.guild_id)
    res.json({ success: true, settings })
  } catch (error) {
    console.error('Get suggestion settings error:', error.message)
    res.status(500).json({ error: 'Failed to fetch suggestion settings' })
  }
})

router.put('/', requireSession, requireGuildAccess, async (req, res) => {
  try {
    const guildId = req.params.guild_id
    await upsertSuggestionSettings(guildId, req.body || {})
    const settings = await getSuggestionSettings(guildId)
    await logAuditAction(req.user.id, guildId, 'UPDATE_SUGGESTION_SETTINGS', settings)
    res.json({ success: true, message: 'Suggestion settings updated', settings })
  } catch (error) {
    console.error('Update suggestion settings error:', error.message)
    res.status(500).json({ error: 'Failed to update suggestion settings' })
  }
})

export default router
