<template>
  <div class="config">
    <header class="config__head">
      <div class="config__eyebrow">{{ t('tictactoe.eyebrow') }}</div>
      <h1 class="config__title">{{ t('tictactoe.title') }}</h1>
      <p class="config__sub">{{ t('tictactoe.sub') }}</p>
    </header>

    <section class="form-card">
      <div class="row row--toggle">
        <div>
          <div class="row__label">{{ t('tictactoe.enableLabel') }}</div>
          <div class="row__hint">{{ t('tictactoe.enableHint') }}</div>
        </div>
        <AppToggle v-model="form.enabled" />
      </div>

      <div class="row">
        <label class="row__label">{{ t('tictactoe.channelLabel') }}</label>
        <div class="row__hint">{{ t('tictactoe.channelHint') }}</div>
        <ChannelSelector v-model="form.games_channel_id" :guild-id="guildId" :types="['text']" />
      </div>

      <div class="form-card__note form-card__note--info">{{ t('tictactoe.usageNote') }}</div>
    ...
    <section class="form-card lb-card">
      <h2 class="lb-card__title">{{ t('tictactoe.leaderboardTitle') }}</h2>
      <div v-if="leaderboard.length === 0" class="lb-empty">{{ t('tictactoe.leaderboardEmpty') }}</div>
      <table v-else class="lb-table">
        <thead>
          <tr><th>#</th><th>{{ t('tictactoe.playerCol') }}</th><th class="num">{{ t('tictactoe.winsCol') }}</th><th class="num">{{ t('tictactoe.playsCol') }}</th></tr>
        </thead>
        <tbody>
...
async function load() {
  ...
  try {
    ...
    toast.error(t('tictactoe.loadError'))
  }
  try {
