<template>
  <div class="config">
    <header class="config__head">
      <div class="config__eyebrow">{{ t('poker.eyebrow') }}</div>
      <h1 class="config__title">{{ t('poker.title') }}</h1>
      <p class="config__sub">{{ t('poker.sub') }}</p>
    </header>

    <section class="form-card">
      <div class="row row--toggle">
        <div>
          <div class="row__label">{{ t('poker.enableLabel') }}</div>
          <div class="row__hint">{{ t('poker.enableHint') }}</div>
        </div>
        <AppToggle v-model="form.enabled" />
      </div>

      <div class="row">
        <label class="row__label">{{ t('poker.channelLabel') }}</label>
        <div class="row__hint">{{ t('poker.channelHint') }}</div>
    ...
    <section class="form-card lb-card">
      <h2 class="lb-card__title">{{ t('poker.leaderboardTitle') }}</h2>
      <div v-if="leaderboard.length === 0" class="lb-empty">{{ t('poker.leaderboardEmpty') }}</div>
      <table v-else class="lb-table">
        <thead>
          <tr><th>#</th><th>{{ t('poker.playerCol') }}</th><th class="num">{{ t('poker.winsCol') }}</th><th class="num">{{ t('poker.playsCol') }}</th></tr>
        </thead>
        <tbody>
