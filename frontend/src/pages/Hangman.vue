<template>
  <div class="config">
    <header class="config__head">
      <div class="config__eyebrow">{{ t('hangman.eyebrow') }}</div>
      <h1 class="config__title">{{ t('hangman.title') }}</h1>
      <p class="config__sub">{{ t('hangman.sub') }}</p>
    </header>

    <section class="form-card">
      <div class="row row--toggle">
        <div>
          <div class="row__label">{{ t('hangman.enableLabel') }}</div>
          <div class="row__hint">{{ t('hangman.enableHint') }}</div>
        </div>
        <AppToggle v-model="form.enabled" />
      </div>

      <div class="row">
        <label class="row__label">{{ t('hangman.channelLabel') }}</label>
        <div class="row__hint">{{ t('hangman.channelHint') }}</div>
        <ChannelSelector v-model="form.games_channel_id" :guild-id="guildId" :types="['text']" />
      </div>

      <div class="form-card__note form-card__note--info">{{ t('hangman.usageNote') }}</div>
    ...
    <section class="form-card lb-card">
      <h2 class="lb-card__title">{{ t('hangman.leaderboardTitle') }}</h2>
      <div v-if="leaderboard.length === 0" class="lb-empty">{{ t('hangman.leaderboardEmpty') }}</div>
      <table v-else class="lb-table">
        <thead>
          <tr><th>#</th><th>{{ t('hangman.playerCol') }}</th><th class="num">{{ t('hangman.winsCol') }}</th><th class="num">{{ t('hangman.playsCol') }}</th></tr>
        </thead>
        <tbody>
...
async function load() {
  ...
  try {
    ...
    toast.error(t('hangman.loadError'))
  }
  try {
