<template>
  <div class="config">
    <header class="config__head">
      <div class="config__eyebrow">{{ t('trivia.eyebrow') }}</div>
      <h1 class="config__title">{{ t('trivia.title') }}</h1>
      <p class="config__sub">{{ t('trivia.sub') }}</p>
    </header>

    <section class="form-card">
      <div class="row row--toggle">
        <div>
          <div class="row__label">{{ t('trivia.enableLabel') }}</div>
          <div class="row__hint">{{ t('trivia.enableHint') }}</div>
        </div>
        <AppToggle v-model="form.enabled" />
      </div>

      <div class="row">
        <label class="row__label">{{ t('trivia.channelLabel') }}</label>
        <div class="row__hint">{{ t('trivia.channelHint') }}</div>
        <ChannelSelector v-model="form.games_channel_id" :guild-id="guildId" :types="['text']" />
      </div>

      <GameLanguagePicker v-model="form.games_language" />

      <div class="form-card__note form-card__note--info">{{ t('trivia.usageNote') }}</div>
    ...
    <section class="form-card lb-card">
      <h2 class="lb-card__title">{{ t('trivia.leaderboardTitle') }}</h2>
      <div v-if="leaderboard.length === 0" class="lb-empty">{{ t('trivia.leaderboardEmpty') }}</div>
      <table v-else class="lb-table">
        <thead>
          <tr><th>#</th><th>{{ t('trivia.playerCol') }}</th><th class="num">{{ t('trivia.winsCol') }}</th><th class="num">{{ t('trivia.playsCol') }}</th></tr>
        </thead>
        <tbody>
...
async function load() {
  ...
  try {
    ...
    toast.error(t('trivia.loadError'))
  }
  try {
