  ...
  <div class="admin">
    <div class="admin__inner">
      ...
      <div v-if="showToolbar" class="admin__toolbar">
        ...
        <AppButton v-if="tab === 'errors' && errors.length && can('errors', 'manage')" variant="danger" @click="clearErrors">
          {{ t('admin.errClear') }}
        </AppButton>
      </div>
        ...
        <template v-else>
          ...
          <li v-for="u in users" :key="u.discord_id" class="row" :class="{ 'is-blocked': u.blocked, 'is-censored': u.blocked && !revealBlocked }">
            ...
            <div class="row__right">
              ...
              <AppButton v-if="!isMe(u.discord_id) && can('users', 'manage')" :variant="u.blocked ? 'subtle' : 'danger'" :loading="busyId === u.discord_id" @click="u.blocked ? unblockUser(u) : askBlockUser(u)">
        ...
        <template v-else>
          ...
          <li v-for="g in guilds" :key="g.id" class="row" :class="{ 'is-blocked': g.blocked, 'is-censored': g.blocked && !revealBlocked }">
            ...
            <div class="row__right">
              ...
              <select v-if="can('guilds', 'manage')" class="tier-select" :class="`tier-select--${g.premium_tier || 'free'}`" :value="g.premium_tier || 'free'" :disabled="busyId === g.id" :title="t('admin.tierTitle')" @change="setPremium(g, $event.target.value)">
              ...
              <AppButton v-if="can('guilds', 'manage')" :variant="g.blocked ? 'subtle' : 'danger'" :loading="busyId === g.id" @click="g.blocked ? unblockGuild(g) : askBlockGuild(g)">
      ...
      <div v-else-if="tab === 'premium'">
        ...
        <div v-if="can('premium', 'manage')" class="panel panel--form">
          ...
          <ul v-if="codes.length" class="rows" style="margin-top: var(--space-4)">
            <li v-for="c in codes" :key="c.code" class="row">
              ...
              <div class="row__right" v-if="can('premium', 'manage')">
      ...
      <div v-else-if="tab === 'jobs'">
        ...
        <ul v-else class="rows">
          <li v-for="j in jobs" :key="j.id" class="row">
            ...
            <div class="row__right">
              <AppButton v-if="j.status === 'failed' && can('jobs', 'manage')" variant="subtle" :loading="busyId === j.id" @click="retryJob(j)">{{ t('admin.jobsRetry') }}</AppButton>
            </div>
          </li>
        </ul>
      ...
      <div v-else-if="tab === 'marketplace'">
        <div v-if="can('marketplace', 'manage')" class="panel panel--form">
        ...
        <ul v-else class="rows">
          <li v-for="tp in templates" :key="tp.id" class="row">
            ...
            <div class="row__right">
              <select v-if="can('marketplace', 'manage')" :value="tp.status" class="filter__select" @change="setTemplateStatus(tp, $event.target.value)">
              ...
              <AppButton v-if="can('marketplace', 'manage')" variant="danger" :loading="busyId === tp.id" @click="deleteTemplate(tp)">{{ t('admin.mpDelete') }}</AppButton>
            </div>
          </li>
        </ul>
      </div>
      ...
      <div v-else-if="tab === 'team'">
        <div v-if="can('team', 'manage')" class="panel panel--form">
      ...
      <div v-else-if="tab === 'partners'">
        <div v-if="can('partners', 'manage')" class="panel panel--form">
      ...
      <div v-else-if="tab === 'changelog'">
        <div v-if="can('changelog', 'manage')" class="panel panel--form">
        ...
        <div class="panel">
          ...
          <ul v-else class="rows">
            <li v-for="e in changelog" :key="e.id" class="row">
              ...
              <div class="row__right" v-if="can('changelog', 'manage')">
      ...
      <div v-else-if="tab === 'system'">
        <div class="panel panel--form">
          ...
          <div class="panel__actions">
            <AppButton v-if="can('system', 'manage')" variant="primary" :loading="savingMaintenance" @click="saveMaintenance">{{ t('admin.sysSave') }}</AppButton>
          </div>
        </div>
        ...
        <div class="panel panel--form">
          ...
          <div class="panel__actions">
            <AppButton v-if="can('system', 'manage')" variant="primary" :loading="savingAnnouncement" @click="saveAnnouncement">{{ t('admin.sysSave') }}</AppButton>
          </div>
        </div>
        ...
        <div class="panel panel--form">
          ...
          <div class="panel__actions">
            <AppButton v-if="can('system', 'manage')" variant="primary" :loading="sendingBroadcast" :disabled="!broadcastMessage.trim()" @click="sendBroadcast">{{ t('admin.bcSend') }}</AppButton>
          </div>
      ...
      <div v-else-if="tab === 'staff'">
        <div class="panel panel--form">
          <h3 class="panel__title">{{ isEditingStaff ? t('admin.staffEditTitle') : t('admin.staffTitle') }}</h3>
          <p class="panel__desc">{{ t('admin.staffDesc') }}</p>
          <div class="team-form">
            <div class="team-form__grid">
              <label class="tf"><span class="modal__label">{{ t('admin.staffUserId') }}</span><input v-model="staffForm.user_id" class="modal__input" :placeholder="t('admin.teamDiscordIdPlaceholder')" /></label>
              <label class="tf"><span class="modal__label">{{ t('admin.staffNote') }}</span><input v-model="staffForm.note" class="modal__input" maxlength="80" :placeholder="t('admin.staffNotePlaceholder')" /></label>
            </div>
            <div class="staff-perms">
              <div class="staff-perms__head">{{ t('admin.staffPermsLabel') }}</div>
              <div v-for="pt in ALL_TABS" :key="pt" class="staff-perm">
                <span class="staff-perm__name">{{ t(`admin.tab${cap(pt)}`) }}</span>
                <div class="seg staff-perm__seg">
                  <button v-for="lvl in ['none', 'view', 'manage']" :key="lvl" class="seg__btn" :class="{ 'is-active': staffLevel(pt) === lvl }" @click="setStaffLevel(pt, lvl)">{{ t(`admin.staffLvl${cap(lvl)}`) }}</button>
                </div>
              </div>
            </div>
            <div class="team-form__actions">
              <AppButton v-if="isEditingStaff" variant="ghost" @click="resetStaffForm">{{ t('common.cancel') }}</AppButton>
              <AppButton variant="primary" :loading="staffSaving" @click="saveStaff">{{ t('admin.staffSave') }}</AppButton>
            </div>
            <p class="admin__hint">{{ t('admin.staffHint') }}</p>
          </div>
        </div>

        <div class="panel">
          <h3 class="panel__title">{{ t('admin.staffListTitle') }}</h3>
          <p v-if="!staff.length" class="panel__empty">{{ t('admin.staffEmpty') }}</p>
          <ul v-else class="team-list">
            <li v-for="s in staff" :key="s.user_id" class="team-list__row">
              <div class="team-list__av" :style="s.avatar_url ? { backgroundImage: `url('${s.avatar_url}')` } : {}">
                <span v-if="!s.avatar_url">{{ initials(s.username || s.user_id) }}</span>
              </div>
              <div class="team-list__meta">
                <span class="team-list__name">{{ s.username ? '@' + s.username : s.user_id }}<template v-if="s.note"> · {{ s.note }}</template></span>
                <span class="team-list__role">{{ s.user_id }} · {{ t('admin.staffPermCount', { n: staffPermCount(s) }) }}</span>
              </div>
              <button class="linkbtn" @click="editStaff(s)">{{ t('admin.teamEdit') }}</button>
              <button class="linkbtn linkbtn--danger" @click="removeStaff(s)">{{ t('common.delete') }}</button>
            </li>
          </ul>
        </div>
      </div>
      ...
      <transition name="modal">
        <div v-if="inspect" class="modal-overlay" @click.self="inspect = null">
          <div class="modal modal--wide">
            ...
            <h4 v-if="can('guilds', 'manage')" class="insp-sub">{{ t('admin.inspectPremium') }}</h4>
            <div v-if="can('guilds', 'manage')" class="insp-premium">
...
const ALL_TABS = ['overview', 'analytics', 'premium', 'health', 'users', 'guilds', 'audit', 'jobs', 'errors', 'marketplace', 'team', 'partners', 'changelog', 'system']
const access = reactive({ is_owner: false, is_staff: false, permissions: {} })
function can(t, level = 'view') {
  if (access.is_owner) return true
  const have = access.permissions?.[t]
  if (!have) return false
  return level === 'view' ? (have === 'view' || have === 'manage') : have === 'manage'
}
const visibleTabs = computed(() => {
  const list = ALL_TABS.filter((t) => can(t, 'view'))
  if (access.is_owner) list.push('staff')
  return list
})
...
const TAB_ICONS = {
  ...
  staff: '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 11l-3 3-1.5-1.5"/>'
}
function tabIcon(tb) {
...
async function loadAccess() {
  try {
    ...
    access.is_owner = !!data.is_owner
    access.is_staff = !!data.is_staff
    access.permissions = data.permissions || {}
  } catch { /* router guard already blocks non-admins; keep defaults */ }
  // Land on the first tab the user can actually see.
  if (!visibleTabs.value.includes(tab.value)) {
    tab.value = visibleTabs.value[0] || 'overview'
  }
}
onMounted(async () => { await loadAccess(); await load() })
...
function emptyStaffForm() {
  return { user_id: '', note: '', permissions: {} }
}
const staffForm = reactive(emptyStaffForm())
const isEditingStaff = computed(() => staff.value.some((s) => s.user_id === staffForm.user_id))
function resetStaffForm() { Object.assign(staffForm, emptyStaffForm()) }
function editStaff(s) {
  Object.assign(staffForm, { user_id: s.user_id, note: s.note || '', permissions: { ...s.permissions } })
  if (typeof window !== 'undefined') window.scrollTo({ top: 0, behavior: 'smooth' })
}
function staffLevel(t2) { return staffForm.permissions[t2] || 'none' }
function setStaffLevel(t2, level) {
  const p = { ...staffForm.permissions }
  if (level === 'none') delete p[t2]
  else p[t2] = level
  staffForm.permissions = p
}
function staffPermCount(s) { return Object.keys(s.permissions || {}).length }
async function saveStaff() {
  if (!staffForm.user_id.trim()) { toast.error(t('admin.staffUserRequired')); return }
  staffSaving.value = true
  try {
    const { data } = await api.post('/admin/staff', {
      user_id: staffForm.user_id.trim(), note: staffForm.note.trim(), permissions: staffForm.permissions
    })
    if (data?.member) {
      const i = staff.value.findIndex((x) => x.user_id === data.member.user_id)
      if (i >= 0) staff.value.splice(i, 1, data.member); else staff.value.push(data.member)
    }
    resetStaffForm()
    toast.success(t('admin.staffSaved'))
  } catch (err) {
    const e = err.response?.data?.error
    toast.error(e === 'is_owner' ? t('admin.staffIsOwner') : e === 'invalid_user' ? t('admin.staffInvalidUser') : t('admin.actionFailed'))
  } finally { staffSaving.value = false }
}
async function removeStaff(s) {
  if (typeof window !== 'undefined' && !window.confirm(t('admin.staffDeleteConfirm', { user: s.username || s.user_id }))) return
  try {
    await api.delete(`/admin/staff/${s.user_id}`)
    staff.value = staff.value.filter((x) => x.user_id !== s.user_id)
    if (staffForm.user_id === s.user_id) resetStaffForm()
  } catch (err) {
    toast.error(err.response?.data?.error || t('admin.actionFailed'))
  }
}
...
.staff-perms { border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--space-3); display: flex; flex-direction: column; gap: 6px; }
.staff-perms__head { font-size: 0.72rem; font-weight: 700; letter-spacing: 0.06em; text-transform: uppercase; color: var(--color-text-soft); margin-bottom: 2px; }
.staff-perm { display: flex; align-items: center; justify-content: space-between; gap: var(--space-3); padding: 4px 0; }
.staff-perm__name { font-size: 0.9rem; font-weight: 600; text-transform: capitalize; }
.staff-perm__seg { flex-shrink: 0; }
