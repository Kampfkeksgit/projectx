<template>
  <div class="config">
    <header class="config__head">
      <div class="config__eyebrow">{{ t('connect4.eyebrow') }}</div>
      <h1 class="config__title">{{ t('connect4.title') }}</h1>
      <p class="config__sub">{{ t('connect4.sub') }}</p>
    </header>

    <section class="form-card">
      <div class="row row--toggle">
        <div>
          <div class="row__label">{{ t('connect4.enableLabel') }}</div>
          <div class="row__hint">{{ t('connect4.enableHint') }}</div>
        </div>
        <AppToggle v-model="form.enabled" />
      </div>

      <div class="row">
        <label class="row__label">{{ t('connect4.channelLabel') }}</label>
        <div class="row__hint">{{ t('connect4.channelHint') }}</div>
    ...
    <section class="form-card lb-card">
      <h2 class="lb-card__title">{{ t('connect4.leaderboardTitle') }}</h2>
      <div v-if="leaderboard.length === 0" class="lb-empty">{{ t('connect4.leaderboardEmpty') }}</div>
      <table v-else class="lb-table">
        <thead>
          <tr><th>#</th><th>{{ t('connect4.playerCol') }}</th><th class="num">{{ t('connect4.winsCol') }}</th><th class="num">{{ t('connect4.playsCol') }}</th></tr>
        </thead>
        <tbody>
...
async function load() {
  ...
  try {
    ...
    toast.error(t('connect4.loadError'))
  }
  try {
